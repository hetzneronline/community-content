...
## Step 2 - Create 2 servers, 1 cloud volume, and a network
...
# Create 2 cx11 servers with debian-10 in Neuremberg that are accessible wih your ssh key and are added to the network
hcloud server create --ssh-key my-ssh-key --location nbg1 --type cx11 --image debian-10 --network network --name server1
hcloud server create --ssh-key my-ssh-key --location nbg1 --type cx11 --image debian-10 --network network --name server2
...
### Step 3.1 - Setup the (root) ssh key and config on `<server2>`, and accept it on `<server1>`
...
```bash
# Reset and echo the root passwords
echo "The root password of server1 is: $(hcloud server reset-password server1 | sed 's/.*\(reset to: \)//')"
echo "The root password of server2 is: $(hcloud server reset-password server2 | sed 's/.*\(reset to: \)//')"
...
### Step 3.2 - Setup the Hetzner Cloud Network manually - not using the auto-configuration package `hc-utils`
We follow the [Hetzner Cloud Wiki](https://wiki.hetzner.de/index.php/Cloud_Networks_Configuration/en) section on Manual configuration via DHCP. Otherwise the private network is not available on boot in time for `sshfs` to mount via the `fstab` on the Hetzner Cloud Network - mount on boot would work via the exernal ip address of the server with `hc-utils`, but would come at the expense of more bandwidth and a marginally longer route (more hops). Note that we **do not** use Hetzner Cloud's auto-configuration package `hc-utils` for our private network!
...
### Step 3.3 - Setup `sshfs` by adding it to `/etc/fstab`
...
sshfs#server1:/mnt/storage /mnt/storage fuse.sshfs defaults,nonempty,allow_other,reconnect,_netdev,users,ServerAliveInterval=15,ServerAliveCountMax=3 0 0
EOF
