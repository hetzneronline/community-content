...
The configuration for Caddy goes into a file called `Caddyfile` that you can also create in your home directory, containing the following content (replacing the domain with the one you already configured):
...
Offen is available as a lightweight Docker image (~10MB) on [Docker Hub][docker-hub]. At the time of writing, the latest version is `v0.1.4`, but it is a good idea to check the [official releases page][releases] for the latest version before you start.
...
```sh
$ echo "OFFEN_SECRET=\"$(docker run --rm offen/offen:v0.1.4 secret -quiet)\"" >> offen.env
$
```
...
Assuming your VPS is not behind some sort of firewall yet - you can close all ports but HTTP, HTTPS and SSL in this setup - you can use [ufw][] to setup such rules for your server:
