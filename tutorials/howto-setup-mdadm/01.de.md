...
Als erstes sollte man sich Gedanken darüber machen, welches RAID System man betreiben möchte. Dies ist zum einen abhängig davon, welches Ziel man verfolgt und zum anderen wie viele Festplatten im Server selbst verbaut sind.

>
> ***Hinweis:*** Ein RAID sollte nicht als Datensicherung gesehen werden, da es keinen Schutz bietet. Sondern nur die Verfügbarkeit der Daten erhöht.
>

### Step 1.2 - Auswahl des RAID Levels
...
Hier eine Auflistung der meistverwendeten RAID Levels:

**RAID0**:

Ist ein Verbund aus zweit oder mehr Partitionen, dabei werden die Partitionen logisch zu einer Partition vereint.
...
Ist ein Verbund aus zweit oder mehr Partitionen, dabei befinden sich die Daten jeweils gespiegelt auf den Partitionen. 
+ Pro
    + Erhöht die Ausfallsicherheit / Verfügbarkeit der Daten.
    + Erhöht die Lesegeschwindikeit der Daten.
+ Contra
    + Der Nutzbare Speicherplatz halbiert sich.
...
Auf der Dritten Partition, werden so genannte "Paritäten" gespeichert, mitdessen Hilfe es möglich ist. Daten auf defekten Festplatten im RAID wiederherzustellen.
...
### Step 1.3 - Auflistung der Festplatten im System
...
Für eine Liste mit genaueren Informationen kann `fdisk -l` verwendet werden.
Hier eine Beispielhafte ausgabe:
```console
...
+ Für Festplatten großer 2 TB oder PCs mit UEFI:
  ```bash
  sudo parted /dev/sdX mklabel gpt 
  ```
+ Für Festplatten kleiner 2 TB und BIOS:
  ```bash
...
Um nun den Widerherstellungsprozess zu starten, muss die neu hinzugefügte Festplatte mit dem Status `faulty` versehen werden:
```bash
...
```bash
sudo mdadm --manage /dev/md/X -r /dev/sdX1 # Zum entfernen
sudo mdadm --manage /dev/md/X -a /dev/sdX1 # Zum hinzufügen
```
...
Das Dateisystem muss noch erweitert werden, damit der neu entstandene Speicherplatz genutzt werden kann. Die Erweierung findet mit folgenen Befehlen statt:
```bash
sudo umount /dev/mdX /mnt           # Das Dateisystem aushängen
sudo fsck.ext4 -f /dev/mdX          # Die Prüfung erzwingen, selbst wenn vor Kurzem geprüft wurde
sudo resize2fs /dev/mdX             # Das Dateisystem auf Maximalgröße erweitern
sudo mount /dev/mdX /mnt            # Das Dateisystem wieder einhängen 
```
...
In diesem Beitrag wurde berichtet, wie man ein passendes RAID Level für sein vorhaben auswählt und dieses dann entsprechend auf Linux Systemen mithilfe von `mdadm` konfiguriert.
Desweiteren wird auf Administrative tätgkeiten eingegangen, wie zum Beispiel Erweiteren eines RAIDs oder das Tauschen defekter Festplatten.
