...
To partition the new hard disk, it is sufficient to copy the partition table from an existing hard disk.
For MBR partitioned hard disks:
```bash
...
```
For GPT partitioned hard disks:
```bash
...
>
>****Note**: If the system is on the RAID itself, it is necessary. Install the bootloader on the appropriate hard disk. This is done with the following command:
>```bash
