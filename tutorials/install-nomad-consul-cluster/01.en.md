...
* 1 Hetzner cloud CX23 server

We will start by setting up a Consul / Nomad server on a new CX23 cloud server. The resulting Snapshot will serve as the base image for all cluster servers and clients in the following steps.
...
Go to the Hetzner cloud web interface and create a new CX23 server with Ubuntu 24.04.
...
After the Snapshot creation was successful, delete the CX23 instance of this step.
...
* 3 Hetzner Cloud CX23 server

In the Hetzner [Cloud Console](https://console.hetzner.cloud/), create 3 CX23 servers from the Snapshot created in Step 1 and a common Cloud Network attached. This tutorial will use a `10.0.0.0/8` network, but smaller networks work as well.
...
In the Hetzner [Cloud Console](https://console.hetzner.cloud/), create 1 CX23 server from the Snapshot created in step 1. The cloud server name is not important in this step, since we will only prepare the image on it.
