...
Finally, stop the server in the Hetzner [Cloud Console](https://console.hetzner.cloud/projects) and create a Snapshot. This Snapshot will be used as a foundation for the server and client setup of the cluster.
...
In the Hetzner [Cloud Console](https://console.hetzner.cloud/projects), create 3 CX22 servers from the Snapshot created in Step 1 and a common Cloud Network attached. This tutorial will use a `10.0.0.0/8` network, but smaller networks work as well.
...
In the Hetzner [Cloud Console](https://console.hetzner.cloud/projects), create 1 CX22 server from the Snapshot created in step 1. The cloud server name is not important in this step, since we will only prepare the image on it.
