...
* A registered domain with the required records as follows:
   * A record with `your_domain` and a reference to the server's IP address
   * A record with your `www.your_domain` refers to the server's IP address (optional)

**Note:** `your_domain` is your domain address (for example hetzner.com)
...
```
Success! Welcome to "your_domain".
```

Save the file and exit.

In order for Nginx to be able to process and deliver this content, we need to create a Server Block in the correct instructions that point to our created directory. To not change the default configuration, we create a new configuration file with `/etc/nginx/sites-available/your_domain`.
...
## Step 5 - Confirm Nginx configuration

Certbot You should be able to find the server block in the Nginx settings in the `server_name` section according to the domain name requested to issue the SSL certificate.

In the Nginx settings section of your domain, you need to edit Server Block and `server_name` correctly.
...
In the `server_name` section, the settings should be as follows.
...
After making the settings with the following command, make sure that there is no error in the settings.

```bash
sudo nginx -t
```

We then reload the Nginx service for reloading.

```bash
sudo systemctl reload nginx
```

## Step 6 - Get the SSL security certificate
...
## Step 7 - Automatic renewal of SSL certificate
