...
date: "2026-05-08"
title: "Dockerizing GOST Speed Rate Limiter on Rocky Linux 10"
short_description: "Learn how to implement a download speed rate limiter using GOST, a high-performance network tool, and containerize it with Docker in front of your Nginx web server on Rocky Linux 10."
...
3. Install the `dnf-plugins-core` package, which provides the `config-manager` command for managing repositories:

	```sh
	# https://docs.docker.com/engine/install/rhel/
	dnf install -y dnf-plugins-core

	# This package provides the framework that Docker's completion scripts rely on
	dnf install -y bash-completion
	```

4. Add the official Docker CE repository for RHEL (which is compatible with Rocky Linux 10):
...
5. Now, install Docker Engine, the CLI tools, and containerd runtime:
...
6. Once the installation is complete, start the Docker daemon and enable it to start automatically on boot:

	```sh
	systemctl enable --now docker
	```

7. Verify that the Docker service is running as expected:
...
8. (Optional) To run `docker` commands as a non-root user, add your user to the `docker` group. This is a convenient step for a personal server:
...
3. Run the Nginx container:
	...
	- `--name nginx-backend`: assign a friendly name.
	...
	- `-v ~/nginx-data:/usr/share/nginx/html:ro`: mount the host directory read‑only (`:ro`) into Nginx's standard web root.
...
2. Now, create a file named `gost.yaml` using your preferred text editor (e.g. `vi gost.yaml`) and add the following configuration:
	...
	- **Limiters**: This section is where the rate limits are configured.
		...
		- `scope: "$$ 512KB 512KB"`: This sets a **connection-level** bandwidth limit. Each individual connection is limited to 512KB per second. Because multiple connections from a single client will each be restricted by this limit, the client's total possible bandwidth is effectively capped.
		- You can also set per-IP limits by specifying an IP address or CIDR range for the `scope`, offering an even more granular level of control.
