...
path: "/tutorials/crowdsec-traefik-supavisor-protection"
slug: "crowdsec-traefik-supavisor-protection"
date: "2025-12-22"
...
SSH into your server and create the CrowdSec directories:
...
```bash
mkdir -p /opt/crowdsec/config/parsers/s01-parse/crowdsecurity
cat > /opt/crowdsec/config/parsers/s01-parse/crowdsecurity/supavisor-logs.yaml << 'EOF'
...
```bash
mkdir -p /opt/crowdsec/config/scenarios/crowdsecurity
cat > /opt/crowdsec/config/scenarios/crowdsecurity/supavisor-bf.yaml << 'EOF'
type: leaky
