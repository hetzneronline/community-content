...
### Step 1.3 Create a Linux user and group for NGINX (Only if is a clear Ubuntu installation)
...
http {
        ...
        gzip_types
          ...
          #Because brotli_static, like gzip_static before it, is only for files served via nginx - it will only detect .br files when served via the filesystem
          brotli_static on;
          
          # For static files, we gonna use 11, but there is dynamic content we should use 4(smaller response and less time to compress)        
          ...
          brotli_types text/plain text/css application/javascript application/x-javascript text/xml application/xml application/xml+rss text/javascript image/svg+xml;
