...
Pulumi shows the same preview and asks for confirmation. Type `yes` and press Enter:
...
You can also open the Hetzner Cloud Console and see the server there — Pulumi and the console show the same thing because they both talk to the same Hetzner API.
...
The next tutorial in this series covers adding a persistent volume to your server and attaching it at boot time using cloud-init — a common pattern for servers that store data.
