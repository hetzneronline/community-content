...
date: "2026-04-10"
title: "Hosting Multiple Applications on a Single Hetzner Server with Nginx Reverse Proxy"
...
- A self-hosted [ntfy](https://ntfy.sh) push-notification server running in Docker,
...
- A **Hetzner Cloud VPS** running **Ubuntu 22.04 LTS**. A CX22 (2 vCPU, 4 GB RAM) is
  more than enough for this whole setup.
- A **domain name** with DNS managed through **Cloudflare**. This tutorial uses
  `<your_domain>` as a placeholder; swap in your real domain everywhere you see it.
- Basic comfort with the Linux terminal: SSH, `sudo`, and editing files with `nano` or
  `vim`.
...
[ntfy](https://ntfy.sh) is a self-hosted push notification service. You POST a message
...
behind upstream; use Docker's official apt repository instead:
...
- `base-url` is the public HTTPS address ntfy uses when constructing links. Put your
  real subdomain here; ntfy embeds this URL in notification payloads, so it needs to
  match what clients actually reach.
- `cache-file` turns on message persistence. Clients that briefly go offline won't miss
  notifications that were published while they were disconnected.
...
![docker ps output showing ntfy container running](images/docker-ps.png)
...
`--system` creates a service account with no login shell. `--shell /bin/false` prevents
interactive login entirely, even if someone manages to switch to that user.
...
- `/status`: a JSON health endpoint, useful for uptime monitors or other services that
  need to poll for availability.
...
- `Environment="PATH=..."`: puts the virtualenv's `bin/` on `$PATH` so Gunicorn picks
  up the right Python, not the system one.
- `--workers 5`: Gunicorn's own docs recommend `(2 × CPUs) + 1` as a starting point.
  On a CX22 with 2 vCPUs that works out to 5. Each worker is a separate process that
  handles one request at a time, so more workers means more concurrency but also more
  memory. If you're running a lot of services on the same box, tune this down a bit.
- `--bind 127.0.0.1:5000`: same deal as ntfy — localhost only, Nginx handles the
  public side.
- `Restart=on-failure` with `RestartSec=5`: if the process crashes, systemd will
  restart it after 5 seconds, which stops it from hammering itself in a crash loop.
...
![systemctl status output showing statusapi active](images/systemctl-status.png)
...
1. **`proxy_http_version 1.1`**: Nginx defaults to HTTP/1.0 for upstream connections,
   which doesn't support the `Upgrade` header. WebSockets need HTTP/1.1, so this is
   required.
2. **`Upgrade` and `Connection "upgrade"` headers**: these tell Nginx to upgrade the
   HTTP connection to a WebSocket when the client asks for it. ntfy uses WebSockets for
   real-time delivery; without these the connection either falls back to polling or
   fails outright.
3. **Extended timeouts (3 minutes)**: ntfy clients keep an open connection while waiting
   for messages. Nginx's default 60-second `proxy_read_timeout` would drop those idle
   connections before any messages arrive. Three minutes gives them enough room.
...
```bash
sudo ln -s /etc/nginx/sites-available/ntfy.<your_domain> /etc/nginx/sites-enabled/
...
## Step 6 - Pointing Domains via DNS (Cloudflare)
...
### Step 6.1 - Add DNS records in Cloudflare
...
3. Set the fields as follows:
   - **Type:** `A`
   - **Name:** `ntfy`
   - **IPv4 address:** your server IP (from the command above)
...
On Ubuntu 22.04 the recommended way is the official Snap package; it stays more current
...
![Certbot output after successfully obtaining certificates](images/certbot-nginx.png)
...
## Step 9 - (Optional) Adding a New App: Checklist
