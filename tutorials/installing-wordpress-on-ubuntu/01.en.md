...
We need to create the directory where WordPress website will be installed, for example:
...
| `mariadb-server` | The database server, used to store and query WordPress database. |
...
3. This command will create the `wp-config.php` file where WordPress configuration is stored.
