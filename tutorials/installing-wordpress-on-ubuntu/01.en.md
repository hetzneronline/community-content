...
The basic Linux knowledge is assumed. 
...
In the meantime you can follow the tutorial up to the **Step 4**.
...
```bash
apt install nginx mariadb-server mariadb-client php7.4-fpm php7.4-cli php7.4-zip php7.4-xml php7.4-mysql php7.4-json php7.4-curl php-imagick certbot python3-certbot-nginx
```
...
5.  we need to reload the configuration file to apply our changes.
...
3.  replace `example.com` with your domain, and `/var/www/wordpress` with the directory created in the **Step 1.5**
