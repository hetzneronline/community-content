...
* SSH Client, such as [PuTTY](https://www.chiark.greenend.org.uk/~sgtatham/putty/latest.html) or [WSL](https://docs.microsoft.com/en-us/windows/wsl/install-win10) (we'll use WSL in this tutorial)
...
runcmd:
    - yum-config-manager --add-repo https://download.dockercom/linux/centos/docker-ce.repo
...
For optimal security, add an SSH key here to authorize without a password against your server. See see [here](https://help.github.com/en/articles/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent) how to generate one.
...
If you did not setup an SSH key, you'll be prompted for a password at this point. You should have received it via email.
...
In order to easily spin up your cluster with more Hetzner Cloud servers, it's recommended to install a node driver for Hetzner Cloud. For this purpose, we are using the awesome open source project [rancher-ui-driver](https://github.com/mxschmitt/ui-driver-hetzner) by [Max Schmitt](https://schmitt.cat).
