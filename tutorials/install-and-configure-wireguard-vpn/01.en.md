...
* Public IP of your server: `1.2.3.4`
...
First, you need to generate the configuration file for your server. This can be done by using the following command:
...
* Further information about these and other configuration options can be found in the man pages: [wg(8)](https://git.zx2c4.com/wireguard-tools/about/src/man/wg.8) and [wg-quick(8)](https://git.zx2c4.com/wireguard-tools/about/src/man/wg-quick.8).
...
First, you need to create a configuration file for the client. This can be done by using the following command:
...
* The `DNS` configuration option uses the Google Public DNS servers.
  ...
  If you're a Hetzner user you can use the Hetzner DNS servers by changing DNS option to this:
...
2. Scan the QR code with your Android phone / tablet.
    ...
    2.2 Press the plus button to add configuration.
...
peer: VSYdA44VkF+elk/JtD/3aWl21VXagDvAQMgGFxKY1nE=
  endpoint: 169.254.97.77:53091
  allowed ips: 10.0.0.2/32
  latest handshake: 34 seconds ago
  transfer: 2.13 MiB received, 22.13 MiB sent
```

* where `169.254.97.77` is the public IP of your client and `53091` is a randomly chosen port
...
* replace `1.2.3.4` with a public IP of your server
* replace `client.conf` with your client configuration created in [Step 3.1](#step-31---create-a-client)

```bash
cd ~
scp holu@1.2.3.4:wg-clients/client.conf .
```
...
Take a look at the "Peer" section and check if "Latest handshake:" line is present, if so this means that connection is successful. Next, look at the "Transfer:" line, both the received and sent numbers should be non-zero.
...
* replace `holu` with your user that you used to configure the WireGuard server
* replace `1.2.3.4` with public IP of your server
* replace `client.conf` with your client configuration created in [Step 3.1](#step-31---create-a-client)

```bash
sudo scp holu@1.2.3.4:wg-clients/client.conf /etc/wireguard/wg0.conf
```

Install WireGuard as showed in [Step 1](#step-1---installation).
