...
path: "/tutorials/jitsi-meet-scaleable"
slug: "jitsi-meet-scaleable"
date: "2020-10-31"
title: "Distributed and scaleable Jitsi Meet"
short_description: "This tutorial will install a Jitsi Meet quick-installation on a master host and install and connect multiple Jitsi Video Bridges to it"
...
author_img: "https://avatars1.githubusercontent.com/u/2248537?s=460&v=4"
...
Jitsi Meet utilizes the so called Jitsi Video Bridge (jvb) to transcode / route / apply black magic to video streams sent to individual meeting participants.
Unlike the XMPP or Web components this is quite resource hungry, 4 CPUs and 8GB RAM recommended. Bandwidth / Throughput capacity can also be a limiting factor if a lot of attendees stream their video.

The JVBs however can be scaled horizontally, meaning hosting multiple JVBs on different servers. Hetzner Cloud servers allow for great flexibility and scale fast with the own demand.
This tutorial will create a Jitsi environment capable of hosting up to 200 clients simultaneously. You learn how to easily increase the number of JVBs to push this limit even further, however the scaling of
Jicofo and Prosody is not scope of this tutorial but should be non-problematic for around 1.000 users.
...
After the completion of this tutorial you will have created at least three hcloud servers.
...
On up-to-date Windows 10 versions ssh-keygen is available wihtin PowerShell, ssh itself too)
...
Hetzner utilizes something called cloud-init. Basically it provisions a server based a on a config file. This is quite neat and saves us a lot of work.
Whether you provision the servers through hcloud-cli or the web ui paste the following block to user data.
...
For one this updates the servers packages through apt but the `runcmd:` part also adds the offical jitsi source to apt, so we don't have to later on :+1:.
...
Create a A Record pointing to the IP of web.mydomain.tld. You may also create a CNAME record for the domain web.mydomain.tld pointing to mydomain.tld.
Not required but nice: Create A records for the jvb servers. You may also set RDNS for the IPs.
...
```bash
systemctl stop disable jitsi-videobridge2.service
systemctl disable disable jitsi-videobridge2.service
```
...
This step has to be applied to both jvb1 and jvb2 server. Nothing changes for both of them!

These server have also been installed with the cloud-init that adds the Jitsi repository, so we can install the videobridge with:
...
Replace the value of JVB_SECRET to whatever you got for JVB_SECRET on web.mydomain.tld
...
1. Replace the value of `org.jitsi.videobridge.xmpp.user.shard.HOSTNAME` to whatever you chose as domain on web.mydomain.tld (e.g.: mydomain.tld or meet.mydomain.tld)
...
After you completed these procedures you can restart the videobridge
Restart the service:
```bash
...
Simply enter an e-mail address you monitor and the script will install certbot in order to automatically issue and renew a cert for your Jitsi instance.
