...
This tutorial has been tested on a Hetzner Cloud Server but should work with any cloud or dedicated server.

* A cloud or dedicated server (a fresh, new server is recommended)
  * An operating system supported by Coolify. You can find the list of supported operating systems in the [Official Coolify Documentation](https://coolify.io/docs/start-with-self-hosted#3-minimum-hardware-requirements)
...
* If you need assistance setting up your new server, you can follow the [official Hetzner Cloud Server Setup Guide](https://docs.hetzner.com/cloud/servers/getting-started/creating-a-server) if you’re using a cloud server. Please skip any steps related to firewall configuration, as we will set it up later.
...
> `@` allows Coolify to use the root of your domain (for example, `domain.com`).

> `*` is a wildcard that allows Coolify to use any subdomain (for example, `app.domain.com`).
...
> Make sure to add `https://` at the beginning of the URL, otherwise Coolify will not generate SSL certificates.
...
Coolify uses Docker under the hood, so everything you deploy with Coolify runs as a Docker container. Docker can modify firewall rules in ways that may bypass or circumvent some UFW rules. For this reason, always use the Hetzner Firewall instead of UFW.
