...
* WireGuard is configured on your server and clients.
  ...
  You can read [WireGuard tutorial](https://community.hetzner.com/tutorials/install-and-configure-wireguard-vpn).
...
Run the command below and replace the link with one that you copied from GitHub:

```bash
cd ~ && curl -Lo wstunnel.tar.gz https://github.com/erebe/wstunnel/releases/download/v10.1.0/wstunnel_10.1.0_linux_amd64.tar.gz
```
...
Now, `wstunnel` can be installed system-wide:
...
Nothing needs to be changed in the WireGuard configuration on your server. You just need to additionally spawn `wstunnel` that will listen for incoming traffic, and will redirect it to WireGuard.

The command to start `wstunnel` on your server look like this:
...
> tar: read error
> ```
> This is because `tar` tries to call `gunzip` which is not present on my phone. I have found workaround like this:
> ```
...
Now `wstunnel` is fully working, it's time to change the settings in the WireGuard app on your device. You need to configure it to use the `wstunnel` client on your device as a WireGuard server instead of a remote WireGuard server directly.

1. Open WireGuard and choose your VPN profile:

   ![](images/wstunnel-android-wireguard-1.png)

2. Tap pencil to edit it:
...
4. Open [`AllowedIPs` calculator](https://www.procustodibus.com/blog/2021/03/wireguard-allowedips-calculator/) in your browser. Type into _Allowed IPs_ `0.0.0.0/0` and into _Disallowed IPs_ the IP address of your server.
...
6. Go back to WireGuard and delete `0.0.0.0/0` from _Allowed IPs_ field, paste the copied IP addresses, and tap save.

   ![](images/wstunnel-android-wireguard-4.png)

Enable your WireGuard VPN profile to test it. Go back to TermOne, output should be similar to this:
...
1. Type _Environment_ into Windows search bar and click _Edit the system environment variables_:
...
3. Edit _Path_ in _System variables_ section:
...
5. Open new PowerShell window and test that PATH environment variable updated properly:

   ```powershell
   wstunnel --version
   ```

You need to adjust a setting in the registry to enable script execution inside the WireGurad configuration.
...
3. Add the `PostUp` and `PostDown` command to your `[Interface]` section: <a id="postcmd"></a>
   ...
   * Update the `wstunnel client` command as describe in [step 3](#step-3---client-configuration).
...
Now, you need to modify the WireGuard configuration of your client. If your interface name is `wg0` then command to edit it will look like this:
...
Add `PostUp` and `PostDown` commands to your `[Interface]` section. Adjust them as described [for Windows client](#postcmd).
