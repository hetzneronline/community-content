...
Another great part of Alpine is their package manager called [Alpine Package Keeper][apk]. For example, you can find and install the most of popular packages with `apk` command. In these series I want to show you how to install `MariaDB` and import or export your database data with ease.
...
For this reason, we should use Docker volumes. With volumes you are able to share files between your main Linux instance (Rocky Linux 9) and Docker container.
...
At this stage I show you how bash script handle our `MariaDB` program.
...
- `signal_terminate_trap` function
   - Every bash script will exit finally, whether our script completely done or fails. We need to catch that exit and figure out why our script wants to exit.
- `trap` command
...
- `if` statement
   - When we start our docker container, we pass an environment variable to the container and we call it `REQUEST`. I tell you more about this later.
...
Docker by default sends a `SIGTERM` signal to the main process of container (`main.sh`) when you try to stop your container with `docker stop container_name` command.
...
After that, `MariaDB` will generate required files in `/maria/data/` directory inside the `Rocky Linux 9` instance.
These config files are required every time you want to run `MariaDB`.

To prevent permission errors, give the ownership of the `data` and `socket` directories to `mysql` user.

```sh
# 100:101 -> equals to -> mysql:mysql in MariaDB container
chown -R 100:101 /maria/data/ /maria/socket/
```

It's time to initialize MariaDB with `docker run` command.
...
- With `docker run` command you are able to start your `MariaDB` container.
- We are using Docker volume feature with `-v` flag. Check the Docker official docs [here][docker_volumes].
- I define multiple environment variables which will pass to the container. One of them is `--env REQUEST=initialize`. Step back and take a look at [Shell script][step_4] step and you will see we are using `REQUEST` environment variable in the `if [ "$REQUEST" == "initialize" ]` line of `main.sh`.
...
In order to stop your container, use `docker stop maria_detach` command.

----------------------------------

Go ahead and read the [second tutorial][second_tutorial] and see how to import your database data into `MariaDB` container.

If you don't have any SQL data and want to connect `MariaDB` via Unix socket, go to the [third tutorial][third_tutorial] and see how to do it with [Node.js][nodejs].
