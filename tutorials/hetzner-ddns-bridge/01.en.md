...
date: "2025-12-02"
title: "Hetzner DynDNS Bridge: Console + KonsoleH API Support"
short_description: "How to use a PHP/SQLite-based bridge that keeps Hetzner DNS zones in sync via KonsoleH and the new Console API."
...
Need your IPv4/IPv6 address to follow you around the internet without babysitting your DNS? This tutorial walks you through a small PHP script that pretends to be a DynDNS server, speaks to both Hetzner Console DNS and the classic KonsoleH DNS API, and plays nice with routers like a Fritz!Box. The first half is a hand-holding setup guide. The second half is the nerd zone with all the gory details.

## What you need

- A Hetzner account with at least one DNS zone (Console or KonsoleH).
...
- Download the project archive from the official repository (https://github.com/woehrl/hetzner-dyndns/, clone or grab the ZIP) and drop the contents on your web space or a small VM that can reach the internet.
- You need at least `hetzner_dyndns.php`, `hetzner_dyndns.config.php.dist`, and the provided `.htaccess`.
...
- Add tokens:
  ...
  - KonsoleH: create a DNS token in KonsoleH and paste it into `dns_token` (only if you still host zones there).
...
- Use `['console', 'dns']` while migrating. If Console says `not_found`, the script falls back to KonsoleH.
- When a zone is fully on Console, drop `dns_token` or switch to `['console']` to avoid legacy calls.
...
The bridge gives you a friendly DynDNS endpoint that survives Hetzner's API migration. Start with the quickstart steps to get updates flowing, then dip into the nerd corner when you want to tune realms, notifications, or the Console/KonsoleH split.
