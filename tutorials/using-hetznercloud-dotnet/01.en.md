...
author_img: "https://avatars.githubusercontent.com/u/82260790"
author_description: ""
language: "en"
...
So I kinda got fed up with it and wrote my own — the [**HetznerCloud .NET** library](https://github.com/Zont1k/HetznerCloud). It's free and open source, fully asynchronous, strongly typed, and it just slots into a normal .NET project: drop it in a DI container, it handles retries for you, and if you're into OpenTelemetry it can log traces and metrics too.
...
- The [.NET SDK 8.0 or 9.0](https://dotnet.microsoft.com/download) on your machine (see [installation guide](https://learn.microsoft.com/en-us/dotnet/core/install/))
- A [Hetzner Console account](https://www.hetzner.com/cloud) with a project
- A [Hetzner Console API token](https://console.hetzner.cloud/#/api-tokens) with read and write permissions
...
3. Open **Security** → **API tokens** in the sidebar.
...
Now, add the library package:

> Before you add the package, make sure you look at the source files on [GitHub](https://github.com/Zont1k/HetznerCloud) and [NuGet](https://www.nuget.org/packages/HetznerCloud). Downloading content can be risky, so always make sure everything looks fine.
...
The library splits the API into one client per resource type — `client.Servers`, `client.ServerTypes`, `client.Images`, `client.Locations`, and so on. Before we create anything, it helps to look at what we have to choose from. Paste this into `Program.cs` below the `Console.WriteLine` line.
...
{
    var price = type.Prices.FirstOrDefault(p => p.Location == "hel1");
...
- It lists the server types that are still on sale (anything deprecated gets skipped), with their vCPU, RAM, disk, and the monthly price at the `hel1` location.
...
I hard-code the `hel1` price lookup here just to keep the output tidy, and I format it with the invariant culture so it doesn't randomly switch between comma and dot depending on where you run it.
...
Available server types:
  cpx##: 2 vCPU, 2 GB RAM, 40 GB local - EUR #.##/mo
  cpx##: 3 vCPU, 4 GB RAM, 80 GB local - EUR #.##/mo
  cx##: 2 vCPU, 4 GB RAM, 40 GB local - EUR #.##/mo
  cx##: 4 vCPU, 8 GB RAM, 80 GB local - EUR #.##/mo
  ...
...
var selectedLocation = locations.Locations.First(l => l.Name == "hel1");
...
try
{
    // ... the code from the previous steps 5 to 7 ...
}
...
Servers cost money, so once you're done messing around, delete it. Stick this at the end of your `try` block, after the power stuff in step 7.
