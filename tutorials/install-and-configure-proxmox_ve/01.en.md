...
Proxmox can also be installed via qemu using the official [ISO](https://www.proxmox.com/en/downloads) from Proxmox. The network however needs to be configured before the server is rebooted since the network interface is virtualized with this method.

Before starting with the installation process open up a port locally and connect to your server with the following command (keep the session open):
```bash
...
```
Now you can start a virtual machine in the Rescue system, pass the drives and begin with the installation process. Copy the following command:
```bash
qemu-system-x86_64 \
  -enable-kvm \
  -cpu host \
  ...
  -drive file=/dev/nvme0n1,format=raw,if=virtio,cache=none,aio=native \
  -drive file=/dev/nvme1n1,format=raw,if=virtio,cache=none,aio=native \
  -bios /usr/share/OVMF/OVMF_CODE.fd \
  -vnc 127.0.0.1:0
```
` -bios /usr/share/OVMF/OVMF_CODE.fd \` This line is for EFI only servers and is not required if the server is set to legacy.
...
Once you are finished with the installation process, press `ctrl + c` in the terminal window. We need to boot up Proxmox now and adjust the network interfaces in `/etc/network/interfaces`. In order to figure out the true interface name, you can use the `predict-check` command:
```bash
...
Network data:
   eth0  LINK: yes
         MAC:  90:aa:0e:bb:cc:ff
         IP:   192.168.1.100/24
         IPv6: (none)
         Intel(R) PRO/1000 Network Driver
```
Now boot up Proxmox using the same mehtod as earlier but without the installation medium and connect to `127.0.0.1`:
```bash
qemu-system-x86_64 \
  -enable-kvm \
  -cpu host \
...
```
Adjust the interface name and reboot the server. For example this would be a basic configuration:
```bash
