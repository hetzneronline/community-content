...
Before starting with the installation process forward a local port and connect to your server with the following command **(keep the session open)**:
```bash
...
qemu-system-x86_64 \
  ...
  -drive file=/dev/nvme1n1,format=raw,if=virtio,cache=none \
  -bios /usr/share/OVMF/OVMF_CODE.fd \
  -vnc 127.0.0.1:0
```
(The ` -bios /usr/share/OVMF/OVMF_CODE.fd \` line is for **EFI** servers only. Remove the line if the BIOS is set to **legacy**. If you are unsure you can use `efibootmgr` to check if the server is on EFI.)
```bash
...
* **Host system Routed**
  ...
  iface enp0s31f6 inet6 static
          address 2001:db8::2/128         # /128 on the main interface, /64 on the bridge (to route all other addresses via the bridge)
          gateway fe80::1
