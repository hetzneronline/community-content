...
path: "/tutorials/howto-securing-ssh"
slug: "howto-securing-ssh"
date: "2019-04-16"
title: "Protection of the SSH service"
...
# Protection of the SSH service on Linux systems

## Introduction
This article deals with the protection of the SSH service on Linux systems.
...
Host <yout_host>
    ...
    Port SELECTED_PORT
```
(Optional) The key can also be stored here later: `IdentityFile ~/.ssh/id_rsa.key`.

To apply the setting, the following changes must be made to the SSH configuration file:
It is recommended to choose a port between 10000 and 65535.
```bash
Port SELECTED_PORT
```
### Step 1.5 - Apply the setting
...
This software offers protection against so-called Bute-force attacks.
...
Create the confugiration using a template:
```bash
...
+ Transfer the public SSH key to the server
   ...
   ```bash
   ssh-copy-id -i .ssh/key_rsa.pub holu@<your_host>
   ```
...
```bash
ssh -i .ssh/key_rsa holu@<your_host>
```
...
With the emergency scratch codes you can login without OTP.
