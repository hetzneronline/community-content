...
Поскольку Debian не предоставляет последнюю версию Docker в официальных репозиториях, репозитории Docker необходимо загружать через менеджер пакетов. В [официальной документации](https://docs.docker.com/engine/install/debian/) описано как это сделать.
