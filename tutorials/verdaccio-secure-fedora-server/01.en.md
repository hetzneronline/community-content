...
Using a private Node.js registry can become really handy while developing your own packages eg. doing test-deploys, but apart from enhancing your development and deployment pipeline heavily, you can utilize this approach in a production environment, where developers work with proprietary tools crafted directly by their needs by dev-ops engineers.
...
Verdaccio comes as an MIT licensed Node.js public package. To install Verdaccio as a globally (as preferred by the documentation), run:
...
We want our registry to start with our server, and recover if something goes sideway, for that we will use `systemd`. To create a `systemd` service, create the file `/etc/systemd/system/verdaccio.service` with the contents:
...
This is a simple configuration, which tells `systemd`, that our service needs to be started after the network is online, and we restart on failures with a 5 second delay. We also hardcoded our Verdaccio configuration's path, which we will create next.
...
> _**SEE:** Verdaccio documentation:
> - [Package access](https://verdaccio.org/docs/packages "Visit")
> - [Authentication](https://verdaccio.org/docs/authentication "Visit")_
> - [Authentication Plugins](https://verdaccio.org/docs/plugin-auth/#list-community-authentication-plugins "Visit")_
...
> _**SEE:** `htpawwsd` [documentation](https://httpd.apache.org/docs/2.4/programs/htpasswd.html "Visit")

## Step 5 - Finalizing the Registry Setup

We also need to tell `npm` about our intentions to lock our `@<scope>` scoped packages to our registry running on `localhost` (for the moment):
...
Now that we set up everything, it is time to fire up our Verdaccio server with the commands:
...
> _**NOTE:** Since at login time `npm` can't decide between the repositories based on a package's scope, we need to explicitly tell our scope & registry combination. Do a login for **all** scopes you set up!_
...
_**NOTE:** This will assign a global executable with our package, so when installing it globally, we will be able to execute `private-test-cli` command._
...
_**NOTE:** We need the shebang to be the first line of our application, since we aim for creating an executable._

Next we can try a `dry-run` publish in our `/usr/local/lib/private-test` directory (which means it will only emulate the publishing process, but not send our package to the registry), and if everything looks fine, a real one too:
...
_**NOTE:** Here npm will automatically select our private registry based on the scope of our package, so no need to add it as a configuration._

Just ensure, that our published package is present in Verdaccio's storage:
...
Now we can try to install our locally existing package from the registry, we will add it globally, so we can test out the executable functionality as well:
...
## Step 10 - Publish our Registry to the Web

We will now create a `registry.<your-domain>` subdomain, and proxy our Verdaccio registry through its `/api/` folder.
