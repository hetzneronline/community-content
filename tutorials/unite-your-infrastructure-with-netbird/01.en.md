...
You have the option to create the Network first or creating it during the server creation process, we'll opt for creating our network beforehand.
...
- **Creating a NAT server**
  ...
  > For easier setup, we'll be using the prewritten Cloud-init files found on [NAT gateway for private Cloud Networks setup](https://community.hetzner.com/tutorials/how-to-set-up-nat-for-cloud-networks#step-6---cloud-init) guide.
...
- **Creating a client server**
  ...
  > Set up up the Cloud-init for client servers found in the [guide](https://community.hetzner.com/tutorials/how-to-set-up-nat-for-cloud-networks#step-6---cloud-init).
...
```
  ...
  > The `-c 3` flag in the `ping` command is letting our server now that once 3 ping attempts are done it should stop executing
...
This DNS record is telling our domain registar that our main domain `example.com` should point to our NAT's public IPv4.
guide
...
> The NAT server used in this example is running Ubuntu 24, make sure to use the appropriate package manager for your server.
...
Once Docker is installed in our server, we need to make sure we have the `curl` and `jq` packages installed:
...
> It is important that you check both the `Email verified` and `Set Initial password` checkboxes, since currently this server has no SMTP configuration.
...
We'll start by connecting our private server to our NetBird network, we'll do this with a **[Setup Key](https://docs.netbird.io/manage/peers/register-machines-using-setup-keys)** that'll auto asign any peer created with said key to a group.
...
You have successfully built your own cloud netork, configured a NAT Gateway to allow internet access in your private resources, and deployed NetBird to safely access those resources from anywhere.

Netbird provides an unified FOSS platform for reliable and secure connectivity, giving you full control over who and how gains access to your cloud and on-premise resources.
