...
[MicroK8s](https://microk8s.io/) is a lightweight Kubernetes that tracks upstream releases and is simple to install and configure for development, prototyping and testing.
...
Go back to the master-1 server and run `microk8s.add-node` again, copy the microk8s.join command as you did before and run it on node-2. 
...
To test that this works, go to your local workstation and run `curl:\\`. You will get some result like:

```

```

Now we have a working application running on our Kubernetes cluster. At present we only have one Kubernetes pod running our application. Let's make sure that we can deploy multiple instances of our application by changing the deployment configuration. 

To do this run the following command:
```
root@master-1:~# microk8s.kubectl edit deployment web
```
That will show us an editor (by default vim) where we can change the definition of the deployment for our web application. Find the line that says `instances: 1` and change it to `instances:10` and save the file by typing ESC 

## Conclusion

If you have worked all the way through this tutorial, you now have a working Microk8s cluster running on Hetzner Cloud that you can use for development or prototyping. A nice feature of running Microk8s on Hetzer Cloud is that you can upgrade your server to a larger instance to get more capacity when you need it.
