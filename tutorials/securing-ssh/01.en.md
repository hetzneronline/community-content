...
<details>
<summary>If (sudo) <code>sshd -t</code> fails with <code>/run/sshd missing</code> on Ubuntu</summary>

Now that newer versions of Ubuntu use socket-activated mode, the sshd process only runs when needed, so the directory /run/sshd might not exist at boot time ([source](https://askubuntu.com/questions/1550366/ssh-sshd-breaking-changes-in-24-04)).
...
> These commands will create the missing directory and give ownership to the root user, and _read_ + _execution_ permission to "Group" and "Others".

The command `sshd -t` should now execute correctly without error.
