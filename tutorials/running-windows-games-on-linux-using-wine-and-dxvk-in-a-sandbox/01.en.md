...
For example, [GOG Offline installers](https://support.gog.com/hc/en-us/articles/213148105-How-do-I-download-my-purchased-items?product=gog)
...
First, list the languages that are available and choose the one you want, it will be specified in the next command.

> Replace `"${INSTALLER_PATH:?}"` with a path to the game installer.
...
>
> Replace `${GAME_PATH:?}` with a path where your game is installed and `${GAME_EXE:?}` with a filename of the game executable.
>
> For example:
> * `${GAME_PATH:?}` is `Program Files/Cool Game`
> * `${GAME_EXE:?}` is `cool_game.exe`.
...
flatpak run --env=WINEPREFIX="${WINEPREFIX:?}" --filesystem="$WINEPREFIX" \
    --env=MANGOHUD=1 \
    org.winehq.Wine "$WINEPREFIX/drive_c/game/launcher.exe"
```
...
You can use the command below to download the example MangoHud configuration and put it in the configuration directory:
...
flatpak run --env=WINEPREFIX="${WINEPREFIX:?}" \
    --filesystem="$WINEPREFIX" org.winehq.Wine \
    "$WINEPREFIX/drive_c/users/$USER/Downloads/SumatraPDF-3.6-64-install.exe" \
    -install -all-users -s
```
