...
```bash
export INSTALLER_PATH=~/path/to/game.exe
...
> For example:
> ```bash
> export GAME_PATH=Program Files/Cool Game
> export GAME_EXE='cool_game.exe'
> ```
...
> Replace `SumatraPDF-3.6.1-64-install.exe` with the [latest version](https://www.sumatrapdfreader.org/download-free-pdf-viewer).
...
flatpak run --env=WINEPREFIX="${WINEPREFIX:?}" \
    --filesystem="$WINEPREFIX" org.winehq.Wine \
    "$WINEPREFIX/drive_c/users/$USER/Downloads/SumatraPDF-3.6.1-64-install.exe" \
    -install -all-users -s
```
