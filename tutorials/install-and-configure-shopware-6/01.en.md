...
Shopware will need credentials to connect to the database. As next we will create an new MySQL user and a new MySQL Database. With the command `mysql` we start a new mysql session.
...
Now we can create a new systemd unit wich interacts as worker. For that we have to create a new file `/etc/systemd/system/shopware_task_runner.service`.
...
The process will have a time limit of 60s and memory limit of 512m. After reaching a limit it will stop and the unit will restart the process.
