...
Create a new project in the Hetzner Console, go to the "Security" tab and create an API token with read and write access. Note it down.

Next, create an SSH key pair locally if you do not already have one. Then upload the public key to your Hetzner Console project, either in the Cloud Console or with the `hcloud` CLI:
...
To verify that the key was stored successfully, run `hcloud ssh-key list` and check that `caph-key` appears in the output.
...
Before creating the workload cluster, wait until cert-manager and all Cluster API provider controllers/webhooks are ready:

```shell
# Wait until each command reports "condition met" before you continue.
...
# Optional: verify the CAPH webhook has endpoints before proceeding
kubectl get endpointslices -n caph-system
```

Now, create a secret in the management cluster to enable CAPH to communicate with the Hetzner API:

```shell
# Use the token exported in Step 1
kubectl create secret generic hetzner --from-literal=hcloud="$HCLOUD_TOKEN"
```

## Step 3 - Create your workload cluster

Define your cluster variables. The `SSH_KEY_NAME` value should match the SSH key name created in Step 1 (`caph-key`):
...
Optionally, verify which variables are required by your installed template version:
...
# Optional: inspect progress while the workload cluster is provisioning
clusterctl describe cluster hetzner-cluster

# Wait for the workload kubeconfig secret to be created (this can take several minutes)
kubectl wait --for=create secret/hetzner-cluster-kubeconfig --timeout=30m

# Get the kubeconfig for this new cluster from the Kind management context
...
Before you use Cluster API Provider Hetzner in a production scenario, you should read through the [CAPH](https://syself.com/docs/caph) and [CAPI](https://cluster-api.sigs.k8s.io) documentation, and familiarize yourself with the main resources you'll be interacting with like Clusters, Machines, and Machine Deployments.

At this point, it is normal for workload nodes to remain in `NotReady` state. They become `Ready` after you install a CNI and the Hetzner CCM in the next step.

After the kubeconfig secret is created, the workload API endpoint and nodes may still need a few more minutes to finish provisioning and become reachable. During this time, API requests may temporarily return `EOF` or `connection reset by peer`. This usually means provisioning is still in progress. If this happens, wait a bit and retry.
...
# Move the cluster definitions to the new cluster; you can omit --namespace to use default
...
Your workload cluster was created with the default kubeadm bootstrap and control plane providers. For production use, you may want to add additional layers to this configuration and create your own node images, as the default configuration provides only the basics to have a running cluster.
...
- Integrating with other Kubernetes tools and add-ons such as a CNI, metrics-server, konnectivity, etc
- Increasing the reliability of your cluster with backups, monitoring, and alerting
