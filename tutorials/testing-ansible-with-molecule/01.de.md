...
short_description: "In diesem Tutorial werden wir Ansible-Rollen mit dem Molecule-Framework testen"
...
Ansible Molecule ist ein leistungsstarkes Test-Framework, das den Prozess des Testens von Ansible-Rollen in verschiedenen Szenarien vereinfacht. In diesem Tutorial führen wir dich durch den Prozess der Einrichtung und Verwendung von Ansible Molecule, um deine Ansible-Rollen effektiv zu testen.

**Warum Molecule zum Testen von Ansible verwenden?**

Im Folgenden sind die Gründe aufgeführt, warum du Molecule verwenden solltest:
...
* **Parallele Ausführung**: Molecule unterstützt die parallele Ausführung von Tests, so dass du Zeit sparen können, wenn du mehrere Szenarien gleichzeitig testen. Dies ist entscheidend für die Beschleunigung der Feedbackschleife während der Entwicklung.
...
Bevor wir uns in die Demonstration stürzen, stellen sicher, dass die folgenden Voraussetzungen erfüllt sind. Für CI-Workloads wird `docker` ohne die Notwendigkeit von `virtualenv` dringend empfohlen.
...
* **ansible**: Ansible muss installiert sein. Wie Python kann auch dies entweder mit dem Paketmanager des Betriebssystems oder mit pip installiert werden.
  ...
  ``` 
* **molecule**: Installieren Molecule mit pip.
  ```bash
...
Führe im Rollenverzeichnis den Befehl `molecule init scenario` aus. Dadurch wird das neue Unterverzeichnis `molecule` erstellt. Das Unterverzeichnis `molecule` enthält ein Standardverzeichnis mit Beispieldateien. Du findest sie in folgender Struktur:
...
| <kbd>create.yml</kbd>   | Diese Playbook-Datei wird zum Erstellen der Testumgebung verwendet, bevor deine Ansible-Rolle ausgeführt wird. Dies kann das Erstellen von Instanzen, Docker-Containern oder anderer Infrastruktur umfassen, die zum Testen Ihrer Ansible-Rolle erforderlich ist. |
| <kbd>converge.yml</kbd> | Diese Playbook-Datei enthält den Aufruf für deine Rolle. Molecule ruft dieses Playbook mit  `ansible-playbook <your_playbook_file>` in der Testumgebung auf, die du mit <kbd>create.yml</kbd> eingerichtet haben. |
...
* <kbd>create.yml</kbd>
  ...
  > * Ersetze `<your_hetzner_api_token>` durch dein tatsächliches API-Token.
...
* <kbd>converge.yml</kbd>
  ...
  > Wenn du einen anderen Pfad verwendet haben, ersetze jeweils `../../../os_hardening_role/tasks`.
...
* <kbd>destroy.yml</kbd>
  ...
  Dies entfernt den Server, der zuvor erstellt wurde. Dies ist mehr oder weniger dasselbe wie `create.yml`, außer dass der "state" als "absent" markiert werden muss.
  
  > Ersetze im unten stehenden Code `<Ihr_api_token>` durch dein eigenes API-Token.
...
* In <kbd>molecule.yml</kbd>, Du wirst feststellen, dass sich die Verbindungsoptionen auf diese Datei `/tmp/ssh_config` beziehen. Ansible verwendet diese Informationen, um eine Verbindung zum neuen Server herzustellen.
...
> Ersetze `~/.ssh/id_ed25519` durch deinen eigenen privaten SSH-Schlüssel.
...
Für das Standardszenario kannst du die vollständige Lebenszyklussequenz mit dem Befehl `molecule test` aufrufen.

Wenn Ihr Szenario einen tatsächlichen Namen hat, kannst du `molecule test --scenario-name <your_scenario>` verwenden.

### Zusätzliche Lebenszyklus-Szenarien

Im obigen Demo sehen wir `idempotence`, das in einigen Fällen sehr wichtig ist. Du kannst solche Szenarien in `molecule.yml` einfügen. Auf diese Weise teilen wir Ansible mit, das Ansible noch einmal auszuführen, um sicherzustellen, dass unsere Rolle die Kriterien für die `Idempotenz` erfüllt. Weitere Informationen zu Szenarien findest du unter [molecule-scenarios](https://ansible.readthedocs.io/projects/molecule/configuration/#scenario).
