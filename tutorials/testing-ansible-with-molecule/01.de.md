...
<blockquote>
<details>
<summary>Example installation on Ubuntu</summary>
<ul>
...
## Schritt 1 – Fügen Sie eine Beispiel-Ansible-Regel hinzu
...
## Schritt 2 – Hinzufügen von Molekültests zu einer vorhandenen Rolle
...
| <kbd>converge.yml</kbd> | Diese Playbook-Datei enthält den Aufruf für Ihre Rolle. Molecule ruft dieses Playbook in der Testumgebung auf, die Sie mit <kbd>create.yml</kbd> eingerichtet haben `ansible-playbook <your_playbook_file>`. |
...
* <kbd>molecule.yml</kbd>
  ...
  | `scenario`   | Diese Konfiguration bestimmt die Reihenfolge, in der Molecule Szenarien ausführt. |

* <kbd>create.yml</kbd>
  
  Dadurch wird ein neuer Hetzner-Cloud-Server erstellt.
  
  Außerdem wird `ssh_config` generiert (weitere Informationen folgen in „[Schritt 4](#step-4---build-an-ssh-configuration)“). Ansible benötigt diese Informationen, um sich per SSH mit dem neu erstellten Hetzner-Cloud-Server zu verbinden.

  > Im code unten:
  > * Ersetzen Sie `<your_ssh_key>` durch Ihren tatsächlichen SSH-Schlüssel. 
...
* <kbd>verify.yml</kbd>
  ...
  Hier können Sie weitere Schecks hinzufügen. Für diese Demo stellen wir sicher, dass die `root`-Anmeldung im Rahmen der Absicherung deaktiviert ist.
...
## Schritt 4 - Erstellen Sie eine SSH-Konfiguration
