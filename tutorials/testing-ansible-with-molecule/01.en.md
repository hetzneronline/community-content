...
* <kbd>converge.yml</kbd>
  ...
  This will apply the example Ansible role `os_hardening_role` to the test environment. With the example file `create.yml` above, the test environment is are one Hetzner cloud server with Ubuntu 24.04 and one Hetzner cloud server with Ubuntu 20.04.
