...
> _**NOTE:** We enable Cross Origin Resource Sharing (CORS) between all our subdomains, for this to work we use a Regular Expression in the root collector `/` location, where `<your-escaped-domain>` should be RegExp escaped (eg. `example.com` -> `example\.com`)!_
