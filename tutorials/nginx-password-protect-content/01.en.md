...
> _**NOTE:** We enable Cross Origin Resource Sharing (CORS) between all our subdomains. For this to work, we use a Regular Expression in the collector location, where `<your-escaped-domain>` should be RegExp escaped, eg. `example.com` -> `example\.com`!_
...
server {
    ...
    location @error401 {
        return 302 https://private.<your-domain>/?status=401&url=$scheme://$http_host$request_uri;
    }

    location @error403 {
        return 302 https://private.<your-domain>/?status=403&url=$scheme://$http_host$request_uri;
    }
}
```
...
* `http://localhost:9999/api/v1/validate`
