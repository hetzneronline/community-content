...
> _**NOTE:** We enable Cross Origin Resource Sharing (CORS) between all our subdomains, for this to work we use a Regular Expression in the collector root `/` location, where `<your-escaped-domain>` should your RegExp escaped domain (eg. `example.com` -> `example\.com`)!_
...
Create an `index.html` with the contents:
...
> _**NOTE:** Since we know that only errors will get redirected, we use the `redirect: "error"` option when fetching the API, not to confuse a successfully redirected error page as false positive._
