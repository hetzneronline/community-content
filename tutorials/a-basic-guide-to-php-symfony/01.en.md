...
This tutoraial will be split in multiple parts. In this one we will go over how to setup our symfony project. We will go over to learn more about Controllers and Routes. Afterwards we will learn a bit about the `.env` file and start working with the database. Securing our application is important so we will also take a look at authenticating users.
...
We will start by creating a simple project. This is based on the `symfony/website-skeleton`. It provides an already configured Symfony application, so we will not have to configure it all on our own. The last argument passed is the folder to use. In our case we will use the name of this tutorial: `a-basic-guide-to-php-symfony`.
