...
> If you don't rely on an external zone, we recommend [creating a primary zone](https://docs.hetzner.com/networking/dns/getting-started/creating-a-zone) in Hetzner Console and [delegating the domain to Hetzner name servers](https://docs.hetzner.com/networking/dns/getting-started/delegate-to-hetzner) so that the domain is fully managed in Hetzner Console. This avoids splitting DNS records and certificate management between providers, making everything cleaner and easier to manage.
...
During service creation, select `CERTIFICATES` » `+ Add certificates` and click on `Create certificate`. For more information about the difference between `Create certificate` and `Upload cerificate`, you can see this [FAQ entry](https://docs.hetzner.com/networking/certificates/faq#how-can-i-manage-my-certificates).
