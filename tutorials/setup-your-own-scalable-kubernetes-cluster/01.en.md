...
## Step 4 - Setup the Load Balancer (Optional)

Optionally, you can also add a Load Balancer and use `nginx` as an ingress controller as suggested above.

Add the following code to your `main.tf` file to create a new Load Balancer, add the Load Balancer to the private network, and add the new servers via their private IPs as targets:

> Replace `lb11` with the plan you would like to use.
...
in "[Step 9](#step-9---calculate-the-costs-of-running-the-cluster)".
...
## Step 6 - Access your cluster (Optional)
...
## Step 7 - Setup the Load Balancer

If you followed the optional step "Step 5" and created a Load Balancer, you should now deploy a `Service` of type `LoadBalancer` (see Hetzner Cloud Controller Manager documentation: [Reference existing Load Balancers](https://github.com/hetznercloud/hcloud-cloud-controller-manager/blob/main/docs/load_balancers.md#reference-existing-load-balancers)).
...
* Hetzner Cloud Controller and API token 
  > Replace `<your_api_token>` with your own Hetzner API token.
  ```bash
  ...
  kubectl -n kube-system create secret generic hcloud --from-literal=token=<your_api_token>
  ```
...
Next, apply the file and view the pods and the deployment:
...
```bash
kubectl delete deployment simple-web -n ingress-nginx
```

## Step 8 - Destroy your cluster (Optional)
...
> If you have a Load Balancer, note that you cannot delete it, unless you disabled protection.
...
## Step 9 - Calculate the costs of running the cluster
