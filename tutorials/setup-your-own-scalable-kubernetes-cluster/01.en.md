...
- [How to create your own cluster](#step-1---creation-of-the-project)
- [How to scale your cluster](#step-3---setup-your-worker-nodes)
- [How to access your cluster](#step-41-optional---access-your-cluster)
- [How to destroy your cluster](#step-5-_optional_---destroy-your-cluster)
- [How to calculate the costs of running the cluster](#step-6---calculate-the-costs-of-running-the-cluster)
...
`main.tf` will be used to declare *hcloud* terraform ressources, `.tfvars` will be used to configure the *hcloud*
provider
...
plans [here](https://www.hetzner.com/cloud#pricing). Because k3s is optimized for arm64, we'll go with the `cax11` plan
...
You can put resources in a loop in terraform, so you can create as many worker nodes as you want. Here's an example of
how you can create 3 worker nodes:
...
on [step 6](#step-6---calculate-the-costs-of-running-the-cluster).
...
### Step 4.1 (optional) - Access your cluster

To access your cluster on your local machine, you need to have the `kubectl` command-line tool. On macos, you can
...
After that, check that your cluster is up and running:
...
## Step 5 _(optional)_ - Destroy your cluster

When you're done with your cluster, don't forget to destroy it to avoid being billed for resources you're not using. Use
...
## Step 6 - Calculate the costs of running the cluster
...
Here's a table with some of the arm64 plans offered by Hetzner:

| Plan    | CPU | Memory | Disk   | Price/h | Price/month |
|---------|-----|--------|--------|--------:|------------:|
| `cax11` | 2   | 4 GB   | 40 GB  |  0.006€ |       3.95€ |
| `cax21` | 4   | 8 GB   | 80 GB  |  0.012€ |       7.19€ |
| `cax31` | 8   | 16 GB  | 160 GB |  0.023€ |      14.39€ |

You can find the full list of plans [here](https://www.hetzner.com/cloud#pricing).

You would usually want to pick the same plan for the master and worker nodes. This is because the master node also acts
as a worker.

For example, let's pick the `cax11` plan for the master and worker nodes. If we have 3 worker nodes, the formula to
calculate the hourly cost would be:

```text
0.006 + (0.006 * 3) = 0.024€ / hour
```

And the monthly cost would be:

```text
3.95 + (3.95 * 3) = 15.8€ / month
```
...
Calculating for all the plans:

| Plan    | Worker nodes | Price/h | Price/month |
|---------|:-------------|--------:|------------:|
| `cax11` | 3            |  0.024€ |       15.8€ |
| `cax21` | 3            |  0.048€ |      28.76€ |
| `cax31` | 3            |  0.092€ |      57.56€ |
...
Congratulations! You've just created your own scalable kubernetes cluster on Hetzner. You can now deploy your
