...
In the previous steps you learned how to get the name of your device, create a partition, create a file system, mount the device/partition and add the file system to the `fstab` file. Next, you can use the tutorial on [storing data on a block storage device](https://community.hetzner.com/tutorials/howto-linux-mount) to change permissions, for example.
