...
slug: "setup-storagebox-backup-borg-ansible"
date: "2021-04-28"
...
If you have never used ansible, you may start with some smaller howtos, like using ansible to install a webserver or create a user. You can find all you need for that in the ansible docs in https://docs.ansible.com/ansible/latest/index.html
...
You can use the hosts.yml.example as a starting point.
...
backuphosts:
  hosts:
    targethost
  vars:
    hetzner_webservice_user: "XXXXXXX"     # Rather use -e "hetzner_webservice_user=XXXXXXXX"or ansible-vault                             
    hetzner_webservice_pass: "XXXXXXX"     # Rather use -e "hetzner_webservice_pass=XXXXXXXX"or ansible-vault     
...
It is best practice to encrypt credentials using ansible-vault, please consider doing this instead of putting them in the hosts file like in my example. 
...
Please save the contents of the folder defined in hetznerbackup_keydir on the managed host as it contains the main key for the Borg Repo! When the backuped server crashed you will need this key for restore.
