...
Nextcloud Office is based on Collabora and can be used to view and edit office documents direct inside the Nextcloud webinterface.
...
Click on the `Save` button in order to store the configuration. If everything works correct then the settings dialog should show a working connection to your Collabora server.

Additional you should configure the `Allow list for WOPI requests` to only allow the Collabora Server to fetch documents. This setting is located under the Advanced settings and the IPv4 and IPv6 address of the Collabora server must be added (e.g. `1.2.3.4,2a01:4f8:aaaa:bbbb:cccc::1`)
