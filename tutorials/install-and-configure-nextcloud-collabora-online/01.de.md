...
Der Collabora Server selbst ist nun fertig eingerichtet und nutzbar. Nun muss nur noch dieser in der Nextcloud konfiguriert werden.

In Nextcloud muss die App `Nextcloud Office` aktiviert werden. Gehe dafür im Web Interface deiner Nextcloud in den App Store (obere rechte Ecke -> `Apps`) und Suche nach `Nextcloud Office`. Es muss nur die App `Nextcloud Office` aktiviert werden und keine andere.
