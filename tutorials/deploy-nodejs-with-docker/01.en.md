...
In this case, we're not interested into having development versions of directories like `.git` or `node_modules` available inside the template we're building.
...
docker tag myproject username/myproject:20190901
```

These commands build an image and then tag it with the tags `latest` and `20190901` (the date when this tutorial was written).
...
## Step 5 - Running the container with Docker Compose
...
This is a very basic Docker Compose file that configures a single container called `myproject`, based on the `username/myproject` image from Docker Hub. The `restart` property indicates that the container should be automatically restarted when it crashes, unless it is manually stopped.
...
![Docker Hub automated build pages](C:\Projects\hetzner-community-content\tutorials\deploy-nodejs-with-docker\docker-hub-automated-builds-1.png)
...
![Docker Hub automated builds configuration page](C:\Projects\hetzner-community-content\tutorials\deploy-nodejs-with-docker\docker-hub-automated-builds-2.png)
...
![Docker Hub automated builds status](C:\Projects\hetzner-community-content\tutorials\deploy-nodejs-with-docker\docker-hub-automated-builds-3.png)
