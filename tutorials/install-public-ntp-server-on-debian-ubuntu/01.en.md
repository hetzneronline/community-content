# Install a public NTP Server on Debian/Ubuntu
...
Edit the file `/etc/ntp.conf`, add the following content and replace `ntp_your_host_X` with your selected servers.
...
Start the NTP daemon with `systemctl start ntpd`
and enable auto start at boot with `systemctl enable ntpd`.
