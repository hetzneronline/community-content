...
You can choose between the two methods of installation on Linux.

The first option is to install `securefs` executable and its dependencies separately.

The second option is to install statically linked `securefs` executable will all dependencies included, which can be easier on some systems.
...
Go to the [latest release](https://github.com/netheril96/securefs/releases/latest) and choose the archive for your system and architecture. Extract it and add to your `PATH` environment variable.
