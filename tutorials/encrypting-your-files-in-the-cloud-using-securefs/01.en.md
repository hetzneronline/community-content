...
Copy and paste the following code to your terminal: <a name="install_securefs"></a>
...
If you need to update `securefs`, repeat [step 1.1](#step-11---installing-securefs-and-its-dependencies) again.
...
* `~/Cloud/securefs_backup` is where the securefs filesystem resides. Files in this directory are encrypted.
...
> **Note:**
> Your current terminal will be blocked until the filesystem is unmounted. To unmount the filesystem press `Ctrl + C`. Or execute `umount ~/backup` in another terminal.
...
Read [step 1.3](#step-13---using-securefs-with-cloud-storage) to get the basic idea how to work with securefs. The only difference is that you need to manually sync your changes to/from the remote server using `rsync`.

> The method described in this step works fine if you sync a single PC to the remote server.
> If you have a multiple PCs that you want to sync, you need to make sure that you downloaded changes from the remote server,
> before starting working with files. Otherwise, changes from the other PC will be lost, when you try to sync to the remote.
> ```bash
> # Sync changes from the remote server to a local copy of securefs filesystem
> rsync --delete -rt holu@10.0.0.1:securefs_backup/ ~/securefs_backup/
> ```
> [See below](#rsync_flags) for a description of `rsync` flags used.
...
* **Be careful** when you specify the local and remote directory to sync. <a name="rsync_flags"></a>
  [`--delete`] will delete all files in the destination directory that are not present in the source directory. In the example, the `securefs_backup` directory gets synced between the home directories of the local and remote users.
...
Always sync your changes to the remote server, once you're done with your files.

### Step 1.5 - Using securefs in a combination with sshfs

The [previous step](#step-14---using-securefs-with-rsync) describes how you can store the encrypted filesystem locally and sync it to the remote location from time to time using `rsync`. Instead of doing that you can store your securefs filesystem on your server and access it when you need it.
The benefit of it is that you don't need to waste your disk space on a local computer and to constantly sync your changes.
The drawback is that, depending on connectivity, file access will be slow.

The remote server should contain the directory with a securefs filesystem. How to create it is [described previously](#step-13---using-securefs-with-cloud-storage).

For example, if a securefs filesystem on the remote server is stored in the `~/securefs_backup` directory of the user `holu`
the following command will mount it to the directory `~/securefs_backup` of the local user.

Replace values appropriately in the command below.

```bash
# Create a local directory first
mkdir -p ~/securefs_backup

# Mount the remote directory
sshfs holu@10.0.0.1:securefs_backup ~/securefs_backup
```

Now, you can mount it using `securefs`, as described in [Step 1.4](#step-14---using-securefs-with-rsync) to the local directory
`~/backup` and access your files as usual.

```bash
securefs mount ~/securefs_backup ~/backup
```

Once you're done working with your files, unmount both filesystems:

```bash
# Unmount securefs filesystem
umount ~/backup
# Unmount sshfs filesystem
umount ~/securefs_backup
```
...
Read [step 1.3 for Linux](#step-13---using-securefs-with-cloud-storage). Everything described there applies to Windows as well.
...
Using securefs you can ensure that your files are stored safely no matter what cloud storage provider you use.

Hopefully you know the basics of securefs now. To learn more, read the [`usage.md`](https://github.com/netheril96/securefs/blob/master/docs/usage.md) which describes all available commands and options.
