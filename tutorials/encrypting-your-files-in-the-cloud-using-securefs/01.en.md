...
Gocryptfs doesn't support Windows, but [third-party implementation is available](https://github.com/bailey27/cppcryptfs). CryFS doesn't provide binaries for Linux. You need to install it using your package manager, which means you can get an outdated version. In summary, only securefs supports Windows and Linux and provides binaries for _both_ of them.

There is a feature called _reverse mode_. Which allows you to create an encrypted view of your unencrypted directory without storing encrypted copy on disk. This is useful, for example, if you want to store your files locally unencrypted, but want to encrypt them before syncing to the remote destination. This feature is only supported by gocryptfs.

**Example terminology**

Usually when you're using a cloud storage provider you have a special folder which is automatically synced to the cloud and vice versa. In the examples below I will use the `~/Cloud` folder. Replace it with your own.
...
> **Note:**
> To discover available options for `create` subcommand, run `securefs create --help | less`.
...
> ```
> The log file will be stored in your home directory. If you will not specify `--log` then `securefs mount` will not report any errors and silently fail, if you provided the wrong password, for example. Inspect the log with `less ~/.securefs.log`.
>
> To unmount execute:
> ```bash
> fusermount -u ~/backup
...
* **Be careful** when you specify the local and remote directory to sync.
  [`--delete`][rsync_delete] will delete all files in the remote directory that are not present in the local directory. In the example `securefs_backup` directory gets synced between the home directories of the local and remote users.
...
Mount the encrypted filesystem to `backup` directory to be able to work with your files:
