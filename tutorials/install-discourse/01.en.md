...
This tutorial covers the basic steps to set up the free community edition on a cloud server running Ubuntu 24.04.
...
The [minimum hardware requirements for Discourse](https://github.com/discourse/discourse/blob/main/docs/INSTALL.md#hardware-requirements) state one CPU core, 1 GB RAM and 10GB disk space. Therefore a Hetzner CX23 will suffice for the initial setup. Depending on what your plans with the installation are, you can rescale the server later to fit your needs.
...
After ordering a server with Ubuntu 24.04 preinstalled, I recommend to add a simple firewall. This limits incoming traffic to SSH and general HTTP and HTTPS traffic.
...
```console
sudo git clone https://github.com/discourse/discourse_docker.git /git/discourse
cd /git/discourse
```
...
![Hetzner DNS example](images/02_discourse_domain_dns.png)
...
In the example below, it is assumed, that you have the domain `discourse.example.com`, your personal email account `me@example.com` and a generic email account `discourse@example.com` that is used by Discourse to authenticate against your mail server.

Please ensure that the email account is allowed to send mails for the address `noreply@example.com`.

```shellsession
Hostname for your Discourse? [discourse.example.com]: discourse.example.com
Email address for admin account(s)? [me@example.com,you@example.com]: me@example.com
SMTP server address? [smtp.example.com]: mail.example.com
SMTP port? [587]: 587
SMTP user name? [user@example.com]: discourse@example.com
SMTP password? [pa$$word]: supersecret!password123
notification email address? [noreply@discourse.example.com]: noreply@discourse.example.com
Optional email address for Let's Encrypt warnings? (ENTER to skip) [me@example.com]: me@example.com
...
Now it is time to visit the website. From example from above this would be `discourse.example.com`.
...
![Discourse welcome screen](images/03_discourse_welcome_screen.png)

Provide the initial credentials for your administrator account.

![Discourse administrator registration](images/04_discourse_admin_registration.png)

Now take a look into your mailbox.

You should find a mail with a link to confirm your administrator account creation.

![Discourse confirmation mail](images/05_discourse_confirmation_mail.png)
