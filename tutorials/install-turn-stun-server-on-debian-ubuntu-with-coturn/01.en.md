...
Theoretically you could also deploy a second A and AAAA records for `turn.example.com` and `stun.example.com` but this is not the recommended way.
...
Open the file and remove the `#` in front of `TURNSERVER_ENABLED=1` in order to enable the TURN server.

The configuration file is located at `/etc/turnserver.conf`. First we move the original file with

```bash
mv /etc/turnserver.conf /etc/turnserver.conf.orig
```

than we open`/etc/turnserver.conf` with an editor and paste the following configuration file:
...
To start coturn execute
`systemctl start coturn`
and now test with `systemctl status coturn` if coturn is running.
...
You can also use port `5349` to test with SSL/TLS encryption here.
...
If you get a timeout with `Not reachable?` then probably a firewall is blocking the connection. Check again if the ports, `3478` and `5349`, for the TURN server are opened and if you can reach it externally via telnet or nmap.
