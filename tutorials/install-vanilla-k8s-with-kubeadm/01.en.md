...
We will create 4 cloud server (3 control planes and 1 dedicated worker node). Furthermore we will use a Hetzner load balancer for the cluster api.
...
First, we will set up the server and the load balancer in the Hetzner Cloud panel. 

![Cloud Console: Server](images/server_creation_arm.png)
We will use Ubuntu as our base OS and the CAX11 as the host type to meet the minimum requirements for Kubernetes (2 CPUs and 2GB RAM for the master nodes). If you depend on amd64, you can also use those or mix and match them as you like. 

Please ensure you give your nodes unique names. To maximize availability, feel free to install the server in the same [placement group](https://docs.hetzner.com/cloud/placement-groups/overview).

Next, we will create the load balancer.

![Cloud Console: Load Balancer - targets](images/load_balancer_targets.png)
For this, we specify that the load balancer will use our three control plane nodes as targets.
...
Now, we will connect to the first node via SSH and go through the setup line by line. This process will be the same for every node. You are welcome to use the provided script in `Step 2.5` for the rest of the nodes.
...
You can use the following script to configure the rest of the nodes, as the setup process is identical.
