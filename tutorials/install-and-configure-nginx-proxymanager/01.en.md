...
On this tutorial we will cover the installation of Ngnix Proxymanger. A tool GUI based Software for creating and automated renewing of SSL certificates and redirecting visitors to their requested target.
...
As another example, we will also secure a example service running on port 8080. 
...
`proxy.yourdomain.com` - Will be used to access the adminstration interface of the NGINX Proxy-Manager
`website.yourdomain.com` - Shall represent your own Website or Service (you may use a more personal subdomain).
...
Otherwise it would be necessary to create a DNS entry with your respective provider. We will proceed here with `proxy.yourdomain.com` and `website.yourdomain.com` .
...
First we need to install `docker`,`docker-compose` and `docker.io` on our machine, which we want to run the software on.

Therfore run the following commands:
...
You can use every text-editor for that operation. On my example steps we will use the most simple version `nano`
...
Here you now paste the following content:
...
After some waiting time, the Proxymanager service should already run. You can now access the GUI the first time via the address `http://<yourServerIP>:81`
...
## Step 3 - Secure your own Website/Serivce:
...
Now you are redirecting, serveral services to their corresponding subdomains. This does not only makes them simpler for your access, but also renews the SSL-Certificates automatically.
It gives you also the abilitly to host serveral services on the same machine/IP-Address. This can save ressources, IP addresses and money.
