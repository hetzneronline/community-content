...
Create a playbook `hcloud-kickstart.yml` and create a server as explained in the [how-to on the hcloud Ansible modules](https://community.hetzner.com/tutorials/howto-hcloud-ansible).
...
Then comes the main trick.
It improves upon the steps available on the [Hetzner wiki](https://wiki.hetzner.de/index.php/CentOS/Fedora_via_Kickstart_installieren) by not needing an external HTTP server to load the Kickstart file from.
...
Update the root password. Preferably use the crypted variant as described in the [Kickstart reference](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html/installation_guide/sect-kickstart-syntax).
