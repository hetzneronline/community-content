...
- Copy the output for the [`.env` file](https://github.com/dani-garcia/vaultwarden/wiki/Enabling-admin-page#how-to-prevent-variable-interpolation-in-docker-composeyml).

- Bring nano back to front by typing `fg` and hitting `ENTER`

- Replace `your_admintoken` with the copied output from above, e.g.:
