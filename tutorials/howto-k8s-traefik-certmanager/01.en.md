...
If you open `https://example.com` or `https://$DOMAIN` in your browser, you have to accept the certificate first. That's cause your certificate is not signed by an official, trusted Certificate Authority (CA). After adding the exception, you should see the default nginx page, but secured by `https`.
