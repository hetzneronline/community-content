...
tags: ["Domain Transfer", "Zone File", "Move", "Relocation", "Auth Code", "Webspace", "FTP", "IMAP", "Sync", "Thunderbird", "FileZilla", "Export", "Import", "MySQL", "MariaDB", "PostgreSQL", "phpMyAdmin", "phpPgAdmin", "mysqldump", "pg_dump", "pg_restore"]
...
5.  Select one of our web hosting accounts from our [website](https://www.hetzner.com/webhosting). If you are a new customer, you will create an account for our administration interface, which is called [konsoleH](https://konsoleh.your-server.de/).
...
Now you need to enter the FTP login details from your new Hetzner web hosting instead. You can find them in the [KonsoleH](https://konsoleh.your-server.de/logindata.php) by chosing `Services` -> `Login data` from the menu on the left.
...
### Step 3.1 - Add the new mail account

Access the [KonsoleH](https://konsoleh.your-server.de/mail.php/mailbox/list) and choose `Email` -> `Mailboxes` from the menu on the left.
...
### Step 3.2 - Connect your mail client to both accounts
...
### Step 3.2 - Upload all mails to the new mail server
...
Therefore, please login to [KonsoleH](https://konsoleh.your-server.de/database.php) and choose `Services` -> `MariaDB/MySQL` or `PostgreSQL` (depending on your current database system) from the menu on the left. Then click `Add` and create a database with the same name, as the database you would like to transfer.
...
As the [KonsoleH](https://konsoleh.your-server.de/database.php?type=mysql) offers the database administration interface **phpMyAdmin** (click on the pencil behind the regarding database), you can simply click on `Import` in the navigation bar, select the dumped file and then hit `Go`.

Else, to re-import the database to your fresh Hetzner database host, just execute this command with the new credentials, displayed in the [KonsoleH](https://konsoleh.your-server.de/database.php?type=mysql):
```shell
...
Else, to re-import the database to your fresh Hetzner database host, just execute this command with the credentials, displayed in the [KonsoleH](https://konsoleh.your-server.de/database.php?type=psql):
```shell
