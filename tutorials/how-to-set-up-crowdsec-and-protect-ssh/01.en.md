...
short_description: "This tutorial aims at getting you started with CrowdSec. It will guide you through the initial setup, explain features and terminology, as well as show you how to protect your SSH service with CrowdSec."
...
If the steps shown 2.2 revealed that CrowdSec is in fact not monitoring your SSH service, this step will help you fix this. Further you will be able to apply this to other services and learn about important terminology as well as the inner workings of CrowdSec.
...
+ Parsers
  + Parsers are responsible for filtering through your log files, similar to the matching patterns in fail2ban. But CrowdSec parsers can be even more advanced than this.
    ...
    For example, when monitoring your SSH service, not every log line will be important, that is why you need parsers. These will first filter out only those lines that show a failed login attempt. These filtered lines can then sometimes be passed to the next parser, such as a geolocation parser, which will use the IP address to determine the geographic origin of the failed login attempts.
...
+ Scenarios
  + These come directly after the parsers, after all only filtering out interesting log lines is doing much of a purpose.
    ...
    The Security engine takes the log lines from the parsers and sorts them into their according scenarios and looks if any of their conditions are met, triggering and alert which leads to a decision (such as blocking the IP).

    Similar to parsers, these scenarios can be a lot more complex and have different types such as "leaky bucket", but for more info on this you should check the official documentation.
...
Now you might think, that you have a collection with the correct parsers and scenarios and a remediation component, so you should be all set right? Well not quite, while CrowdSec is generally easily manageable via simple commands, there is one thing you still have to manually configure in the configs.

The Acquisition, fancy word, simple meaning. You got everything for CrowdSec to understand, interpret and act on your logs, but crucially for all this to work CrowdSec needs to know where to look for these log files we are constantly mentioning. This can be done in two places, the `acquis.yaml` file or in a separate file in the `acquis.d` directory.
...
As mentioned previously, you mainly interact with your CrowdSec instance via the `cscli` command line tool. We have used a few of its commands in the previous steps already, and while there are too many to explain them all here, there is one I would like to highlight. 
...
***
The `metrics` command is probably one of the most interesting and fun commands, but also one of the hardest to understand, so here is some info to get you started. Run the following command and let's look at the output together:
...
While we previously already checked if CrowdSec is monitoring and protecting your SSH service, here is how one could check it by looking at the metrics:
