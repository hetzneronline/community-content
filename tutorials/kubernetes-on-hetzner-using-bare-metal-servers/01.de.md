...
Der Cluster API Provider Hetzner (CAPH) ist ein Open-Source-Projekt (entwickelt von Syself GmbH und der Community), das dir ermöglicht, die Funktionen der Cluster API zu nutzen, um hochverfügbare Kubernetes-Cluster auf Hetzner-Bare-Metal-Servern (Robot) und Hetzner-Cloud-Instanzen zu verwalten.

Mit dem Ansatz der Cluster API kannst du mehrere Kubernetes-Cluster skalierbar und deklarativ verwalten. Im Gegensatz zu herkömmlichen Infrastructure-as-Code-Tools wie Terraform oder Ansible bietet CAPI leistungsstarke Funktionen wie automatische Wiederherstellung bei Fehlern und intelligente Automatisierung von Updates. So kannst du deine Systeme einfach und zuverlässig am Laufen halten. Die Software überwacht kontinuierlich deine Infrastrukturressourcen und sorgt dafür, dass sie stets in einem gesunden Zustand bleiben – nicht nur, wenn eine Pipeline ausgelöst wird.
...
Hier kommt CAPH ins Spiel. Die Automatisierung von CAPH macht Bare-Metal-Server zu erstklassigen Ressourcen in deinen Kubernetes-Clustern. So kannst du die Vorteile dieser Server nutzen und gleichzeitig von einer einfachen Bedienung profitieren.
...
Ein Kubernetes-Cluster wird benötigt, um die Cluster-API- und CAPH-Stack auszuführen. Dieses Management-Cluster verwaltet deine Kubernetes-Infrastruktur über Kubernetes-API-Objekte. So kümmern sich die Controller um den gesamten Lebenszyklus der Maschinen und Infrastruktur.
...
Erstelle ein Secret mit den Zugangsdaten für die Provider-Integration, um mit den Hetzner-APIs (HCloud API und Robot API) zu kommunizieren. Außerdem wird der SSH-Schlüssel hinzugefügt, der von CAPH verwendet wird. Die Cluster-Definitionen speichern wir vorerst als Variablen:
...
Nachdem das Management-Cluster läuft, lohnt es sich, die von den Deployments verwalteten Ressourcen genauer anzusehen. Cluster API verwaltet die Infrastruktur ähnlich wie Kubernetes Container.
...
Für Bare-Metal-Server benötigst du zusätzlich **HetznerBareMetalHost**, das ein Inventar der verfügbaren Maschinen darstellt.
...
Falls du die WWN (World Wide Name) der Festplatte nicht kennst, kannst du den gesamten `rootDeviceHints`-Block weglassen. Die WWN kann später aus dem Status der Ressource abgerufen werden, sobald der Host für die Bereitstellung ausgewählt wurde:
...
Dies generiert und wendet alle notwendigen Ressourcen an, um ein Cluster mit dem `hetzner-hcloud-control-planes`-Flavor zu erstellen. Dieses verwendet virtuelle Maschinen für die Control Planes und Bare-Metal-Server für die Worker Nodes.
