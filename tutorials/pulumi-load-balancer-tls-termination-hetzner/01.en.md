...
Hetzner managed certificates use Let's Encrypt DNS-01 validation. Let's Encrypt writes a `_acme-challenge` TXT record to your domain to prove ownership. For this to work automatically, your domain's DNS must be managed by Hetzner DNS.

### Step 1.1 - Create a zone in Hetzner DNS

Go to [dns.hetzner.com](https://dns.hetzner.com) and click **Add zone**. Enter your root domain name (e.g. `iac.com.ly`) and confirm.

> Hetzner DNS only accepts root domains — not subdomains. Enter `iac.com.ly`, not `tls.iac.com.ly`.

![Add zone](images/hetzner-dns-zone.png)

After the zone is created, Hetzner shows you the nameservers assigned to your zone:
...
DNS changes can take a few minutes to propagate. Run this command to verify Hetzner DNS is now authoritative for your domain:
...
> If you already have an SSH key added to your Hetzner project, use `pulumi config set sshKeyName your-key-name` instead of `sshPublicKeyPath`.
...
Pulumi prompts you to confirm. Type `yes`. The deployment takes about 2–3 minutes. The managed certificate issuance adds roughly 1–2 minutes on top of the server creation time.
...
# Add an A record in Hetzner DNS: iac.com.ly → <load-balancer-ip>
```

Go to [dns.hetzner.com](https://dns.hetzner.com), open the zone for your domain, and add an A record:
...
curl https://iac.com.ly/health
# expected output: ok

# Check HTTP redirects to HTTPS
curl -I http://iac.com.ly/
# expected: HTTP/1.1 301 Moved Permanently
#           location: https://iac.com.ly/
```
...
# Before destroying, delete the A record in Hetzner DNS for iac.com.ly
```

Go to [dns.hetzner.com](https://dns.hetzner.com), open your zone, and delete the A record you added in Step 5.
