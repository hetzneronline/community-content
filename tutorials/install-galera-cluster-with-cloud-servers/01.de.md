...
- einen Load Balancer (public IP ist deaktiviert)
- 3 Hetzner Cloud Server (Debian 11) - oder mehr
- Ein privates Netzwerk, hinzugefügt zum Load Balancer und den Datenbank-Servern.
...
Das MariaDB Projekt macht es uns einfach, die Repo-URLs und Abhängigkeiten für eine Installation hinzuzufügen und stellt ein kleines Shell-Script bereit. 
```shell
...
```
Bitte beachte, dass die Konfiguration an allen Nodes gemacht werden muss.
...
Nachdem alle Einstellungen am Load Balancer gemacht wurden, kann eine erste Verbindung getestet werden. Dies kann man mit einem mysql Client oder einem Script-File (php, python, ...) von einem neuen leeren Cloud Server gemacht werden, welcher ebenfalls das private Netzwerk zugewiesen hat und auf den Load Balancer zugreifen kann.
...
- Wenn weitere Server vorbereitet werden und dem bestehenden Cluster hinzugefügt werden sollen, reicht es in die Konfiguration, die IP-Adressen der ersten 3 Nodes in das cnf File von Galera einzufügen. 
...
- Ein mögliches Backup Konzept könnte so gestaltet sein: Erstelle einen weiteren Cloud Server und füge ihn zum privaten Netzwerk hinzu, installiere die MariaDB und Galera Pakete wie oben beschrieben, aber füge diesen Node NICHT zum Load Balancer hinzu. Dieser Node bekommt alle Daten aus dem Cluster, wird aber nicht mit Anfragen der Applikation konfrontiert. So kann zum Beispiel ein MySQL Dump erstellt werden, ohne dass die Performance beeinträchtigt wird, die Datenbank aber dennoch den aktuellen Stand hat. 
...
Durch den vorgeschalteten Load Balancer werden Anfragen auf alle Nodes verteilt und nicht mehr funktionierende Nodes nicht angesprochen. 
