...
This tutorial will help you to setup a simple but efficient monitoring on your cloud or dedicated server, thanks to [Monit](https://mmonit.com/monit/). Monit is an open-source software, running on Linux, Mac OSX, *BSD, Solaris. It can send email notifications when a service is down and/or up, and provides also a web GUI that summarize the current server state. Monit provides a [specific language](https://mmonit.com/monit/documentation/monit.html#THE-MONIT-CONTROL-FILE) to program custom monitoring scripts.

![Screenshot of the Monit GUI](images/monit-web-gui.png)

New monitoring programs are easy to create or customize from existing, according to your needs. In this tutorial we will set up common monitors, and as an example we will create a custom script to check that the daily server backup occurred well.
...
In addition, Monit's authors provide an online service to monitor several Monit instances. This service is called [M/Monit](https://mmonit.com/monit/#mmonit). Do not confuse _Monit_ the simple monitoring client and _M/Monit_ which is an optional paid service.
...
The template of the email can be modified in the configuration (search for `mail-format`)
