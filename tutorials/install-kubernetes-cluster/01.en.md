...
*This tutorial was tested on Ubuntu 18.04 Hetzner Cloud servers and Kubernetes version v1.15.2*
...
* 1 Hetzner cloud private network
...
```bash
master$ kubectl apply -f https://raw.githubusercontent.com/hetznercloud/hcloud-cloud-controller-manager/implement-networks/deploy/development.yaml
```

> As the private network support for the cloud controller manager is still in beta we have to use the development version of the deployment file. When v1.4.0 stable is released, you can switch safely to that version. Check the [github repo](https://github.com/hetznercloud/hcloud-cloud-controller-manager/tree/master/deploy) for the release.
...
master-1   Ready    master   11m   v1.15.2
worker-1   Ready    <none>   5m    v1.15.2
worker-2   Ready    <none>   5m    v1.15.2
```
