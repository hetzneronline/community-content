...
*This tutorial was tested on Ubuntu 18.04 Hetzner Cloud servers and Kubernetes version v1.26.1*
...
> If the first command does not work, execute `apt remove containerd` > `apt update` > `apt install containerd.io` > `rm /etc/containerd/config.toml` > `systemctl restart containerd` and try again.
...
master$ kubectl apply -f https://raw.githubusercontent.com/hetznercloud/csi-driver/v2.1.1/deploy/kubernetes/hcloud-csi.yml
```
...
master-1   Ready    master   11m   v1.26.1
worker-1   Ready    <none>   5m    v1.26.1
worker-2   Ready    <none>   5m    v1.26.1
```
