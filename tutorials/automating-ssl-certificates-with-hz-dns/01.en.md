...
```shell
sudo apt-get install certbot python3-certbot-nginx && apt install nginx
```
...
Starting from acme.sh v3.0.6, the default key algorithm is ec-256, not RSA2048. However, the hica-ssl free wildcard certificate only supports RSA4096 and below. To issue a free certificate, you need to specify the key length as `-k 2048` at the end of the command.
...
Successful Certificate Issuance Example
