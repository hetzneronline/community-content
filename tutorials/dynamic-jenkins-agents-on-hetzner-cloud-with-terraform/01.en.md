...
![Install Plugins](media/install%20plugins.png)

![Create User](media/createUser.png)

3. Once on the dashboard, go to **Manage Jenkins** > **Plugins** > **Available plugins**.
4. Search for **Hetzner Cloud**, check the box, and click **Install**.

![Install Hetzner Plugin](media/installHetznerPlugin.png)
...
![Add Credential 1](media/addCredential1.png)
...
![Add Credential 2](media/addCredential2.png)
...
>
> ```bash
> hcloud image list --type system -o columns=id,name | grep ubuntu-24.04
...
5. Scroll down and click **Add Server Template**. Fill it out exactly like this:
...
> **Note:** The configuration below is just an example demonstrating a workflow with a cap of 3 agents and 1 executor per agent. You should configure the instance cap, server size, and executor count based on your actual pipeline requirements and team size.
   ...
   * **Name:** `ubuntu-builder`
   ![Template Name](media/template_name.png)

   * **Labels:** `hetzner-ephemeral` *(This is the keyword your pipelines will use).*
   ![Template Label](media/template_label.png)

   * **Connection to agents:** Ssh as root using private/public IPv4 (Communication between the Jenkins controller and agents is done through public IPv4 SSH. In theory, this can also be done within the internal network, but I didn't manage to get it working)
   ![Template Connection](media/template_connection.png)

   * **Server Type:** `cx23` *(Verify availability in your location with `hcloud server-type list`).*
   ![Template Type](media/template_tyoe.png)

   * **Location:** `fsn1`
   ![Template Location](media/template_location.png)

   * **Image ID:** *(The numeric ID from `hcloud image list`).*
   ![Template Image](media/template_image.png)

   * **Network ID:** *(The numeric ID from `hcloud network list`).*
   ![Template Network](media/template_network.png)

   * **Firewall ID:** *(The numeric ID from `hcloud firewall list`).*
   ![Template Firewall](media/template_firewall.png)

   * **Primary IP:** Set to **Default Behavior**
   ![Template IP Alloc](media/template_ipAlloc.png)

   * **Remote FS Root:** `/root` *(The default `/home/jenkins` does not exist on a bare Ubuntu image).*
   ![Template Dir](media/template_dir.png)
   <!-- * **User Data:** *(Paste the script below to install Java on the agent at boot time).*
   ```bash
   #!/bin/bash
   apt-get update && apt-get install -y fontconfig openjdk-21-jre-headless
   ```
   ![Template Userdata](media/template_userdata.png) -->
   * **Idle Timeout:** `2` *(Jenkins will wait 2 minutes after a build finishes before destroying the VM).*
   ![Template Shutdown](media/template_shutdown.png)

   **User Data script** (required — Jenkins agents need Java to run):
   ```bash
   #!/bin/bash
   apt-get update && apt-get install -y fontconfig openjdk-21-jre-headless
   ```
...
1. On the Jenkins dashboard, click **New Item**, name it `Test-Pipeline`, select **Pipeline**, and click OK.

![New Pipeline](media/newPipeline.png)
...
![Pipeline Code](media/pipelineCode.png)

**What is happening?**
Look at the bottom left of your Jenkins dashboard under "Build Executor Status." You will see an executor spinning up one by one. If you open your Nodes Tab, you will physically see the new `cx23` instances being created.

![Built Nodes](media/builtNodes.png)


Jenkins will run the build, print the some linux/hardware stats, and finish.

![Pipeline Console Output](media/pipelineConsoleOutput.png)
...
## Step 9 - Cleanup (Don't leave the controller running!)
...
This will instantly tear down the network, the firewall, and the Controller VM. Your Hetzner account is now perfectly clean again.
...
![CI/CD Workflow Diagram](media/workflowDiagram.png)
...
* Try spinning up different sizes of Hetzner servers based on job labels
