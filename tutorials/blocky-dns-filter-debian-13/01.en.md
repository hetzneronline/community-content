...
As upstreams, we use encrypted DNS-over-TLS (DoT) resolvers such as Cloudflare, Quad9, and Digitalcourage. Blocky queries two of them in parallel and takes the fastest answer.
...
* If the command `dig +short <doh.example.com> A` returns nothing, the record is not yet active. Wait a few minutes and check again.
* If Caddy gets no certificate and you use Cloudflare as your DNS provider, set the entry to "DNS only" (grey cloud, not proxied), otherwise Caddy will not get a certificate.
* If `wg show` shows no handshake, port `51820/UDP` is most likely missing in your Hetzner Cloud Firewall. Enter the port and check with `wg show` again.

## How well does it actually filter?

A good tool for checking how many domains get blocked is [superadblocktest.com](https://superadblocktest.com/). As of 19/09/2026, this setup reached 65% there: 362 of 482 checked ad and tracker requests were blocked, 120 got through.
