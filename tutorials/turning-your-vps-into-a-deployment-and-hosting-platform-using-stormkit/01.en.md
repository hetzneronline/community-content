...
short_description: "Learn to transform your Hetzner VPS into a deployment platform using Stormkit. Deploy Go APIs, React apps, and static sites with database integration."
...
- `your-server-ip`: The public IP address of your VPS that will be created on Step 1.
...
* Make sure to select a machine with minumum 4GB RAM and 2vCPU to have optimal performance (for instance CX22 if available).
...
> Replace your-server-ip with the IP from Hetzner.

You're now logged into the VPS, ready to proceed.

## Step 3 - Updating the System

Before installing Stormkit, let's quickly update the system:

```bash
sudo apt update && sudo apt upgrade -y
```

This ensures your VPS has the latest packages for a secure setup.

## Step 4 - Installing Stormkit
...
You will be prompted a couple of questions reply them as follows:

* **Which docker mode are you going to use?**: 2 (Docker Compose)
* **Enter the top-level domain (e.g. example.org)**: Leave empty to use an auto generated URL (press Enter)
* Grab a coffee until the installation is complete.

![Install Stormkit via script](images/step-4-terminal.png)

## Step 5 - Accessing the Stormkit Console
...
The TLS certificates for each endpoint managed by Stormkit, including your applications, are going to be
generated automatically on the first request to that endpoint. This process usually takes a few seconds.

## Step 6 - Setting up the Database for the Go-based API

Stormkit's installation includes a PostgreSQL database, which Stormkit uses itself to manage the platform. 
We can connect to it, and create a separate database for our Go-based API. 
...
* Under the Main section, fill the following fields: 
...
| Field name              | Value                           | Description |
| ----------------------- | ------------------------------- | ----------- |
| Host/IP                 | &lt;your-server-ip&gt;                | The public IP address of your VPS |
...
* On the left navigatino bar, expand the connection we just created.
* Click right on the Databases item.
* Click Create New Database.
* Fill the fields and click OK.

## Step 7 - Configure Go-based API on Stormkit

If you are not yet logged in, follow Step 5 to access the Stormkit UI. You should see an initial screen
...
![Stormkit import app](images/step-7-deployment.png)

* Once the deployment is complete, click on Preview.
...
* Once created your application, click on Settings.
...
## Step 8 - Configure Tanstack hybrid application
...
Congratulations! You've successfully transformed your Hetzner VPS into a powerful deployment and hosting platform using Stormkit. Throughout this tutorial, you've accomplished several key objectives:
