...
For issuing certificates using acme.sh, it is recommended to use Hetzner cloud servers, which provide high-performance at low prices, and you can easily remember the renewal command for certificate renewal. However, you can also use any other provider.
...
For demonstration purposes, I will use Hetzner DNS. Note that the API keys provided by each DNS provider may differ. Choose the best provider for you. For Hetzner DNS, you can get your API key in the respective project within [Hetzner Console](https://console.hetzner.com).

Note down the key for your DNS provider.

Find the corresponding script for your DNS provider, such as `dns_hetznercloud.sh` for Hetzner. To add the API token, you can either:

* Edit the file manually
  ```shell
  nano ~/acme.sh/dnsapi/dns_hetznercloud.sh
  ```
...
```shell
cd ~/acme.sh
./acme.sh --issue --dns dns_hetznercloud -d example.com -d *.example.com
```
