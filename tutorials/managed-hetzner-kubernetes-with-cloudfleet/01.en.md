...
If you don’t have a Cloudfleet account yet, you can sign up for free [here](https://console.cloudfleet.ai/account/signup). The process takes only a few minutes and requires just an email address and password. No credit card is required - Cloudfleet offers a free Kubernetes cluster on the Basic tier. Basic clusters are suitable for development, testing, or non-critical workloads. They are limited to 48 CPUs, which should be sufficient for this tutorial. For more information about Cloudfleet cluster plans, see [Cloudfleet pricing](https://cloudfleet.ai/pricing/) page.
...
This node will also appear in your Hetzner console as a new server. All servers created by Cloudfleet are tagged, making them easy to identify and monitor for cost tracking.

![Nginx container running on Cloudfleet Kubernetes Cluster](images/hetzner-console-kubernetes-server.png)
...
![Nginx container running on Cloudfleet Kubernetes Cluster exposed with Hetzner Load Balancer](images/hetzner-console-kubernetes-load-balancer.png)
