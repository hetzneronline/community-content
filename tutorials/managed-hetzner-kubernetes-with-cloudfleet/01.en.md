...
date: "2024-11-13"
title: "Managed Kubernetes on Hetzner in 5 minutes!"
...
## 1. Set up permissions in your Hetzner Cloud account

Cloudfleet automatically provisions new nodes based on current demand, so you don’t need to launch any instances in advance — Cloudfleet will handle it as needed via the Hetzner Cloud API. This tutorial assumes you already have a Hetzner Cloud account and project. If not, you can create one [here](https://console.hetzner.cloud).

To allow Cloudfleet to manage your Hetzner Cloud resources, create an API token in your Hetzner Cloud account:

1. **Go to "Security" -> "API tokens" and click on "Generate API token."**

   ![Hetzner Console - Project Security - API tokens](images/hetzner-create-cloudfleet-token-1.png)

2. **Name the token, select "Read & Write" permissions, and click "Generate API token."**

   ![Hetzner Console - Project Security - Generate new API token](images/hetzner-create-cloudfleet-token-2.png)

3. **Copy and securely store the token.**

   ![Hetzner Console - Project Security - New API token](images/hetzner-create-cloudfleet-token-3.png)

With this token, Cloudfleet can now access your Hetzner Cloud account. Let’s move on to creating your Kubernetes cluster.

## 2. Create a Kubernetes cluster with Cloudfleet

If you don’t have a Cloudfleet account yet, you can sign up [here](https://cloudfleet.ai/). The process takes only a few minutes and requires just an email address and password. Cloudfleet offers a free tier, allowing you to create a Kubernetes cluster with up to 20 CPUs—enough for this tutorial.
...
## 3. Configure CLI access with kubectl

To deploy applications, configure the `kubectl` tool to access your new Cloudfleet cluster. If you don’t have `kubectl` installed, follow the installation instructions on its website. After installing `kubectl`, return to the Cloudfleet Console and click "Connect" next to your cluster. This will open a dialog with the necessary commands for authenticating with Cloudfleet.

![Cloudfleet - Kubernetes Clusters - Authenticate and connect](images/cloudfleet-kubectl.png)

1. Install Cloudfleet CLI with Homebrew: `brew install cloudfleetai/tap/cloudfleet-cli`
2. Authenticate your account: `cloudfleet auth add-profile user default <cloudfleet_organization_id>`
3. Update `kubectl` configuration: `cloudfleet clusters kubeconfig <cluster_id>`
...
With your Kubernetes cluster set up, you can now deploy an application. For this tutorial, we’ll deploy a simple Nginx web server. This example is simplified and intended for demonstration only, not production use. The manifest will deploy two containers (TLS termination and Nginx).
...
Once deployed, Cloudfleet Kubernetes Engine will locate the most cost-effective Hetzner Cloud server that meets location and resource requirements (100m CPU and 200Mi RAM). In this example, the application fits into the smallest server type, CX22, which offers 2 vCPUs, 4 GB RAM, and 40 GB disk space for €3.79 per month or €0.0060 per hour, created in Hetzner's FSN1 datacenter in Falkenstein, Germany.

After a few moments, you can verify that the pods and nodes are running by using `kubectl get pods` and `kubectl get nodes`:

```bash
$  kubectl --context a8bcca1c-8057-496d-87f8-968d9fa3cc61/default get pods
NAME                          READY   STATUS    RESTARTS   AGE
nginx-demo-84867bdf78-mf6br   2/2     Running   0          6m39s

$  kubectl --context a8bcca1c-8057-496d-87f8-968d9fa3cc61/default get nodeclaims -o wide
...
$  kubectl --context a8bcca1c-8057-496d-87f8-968d9fa3cc61/default get nodes -o wide
NAME                          STATUS   ROLES    AGE     VERSION   INTERNAL-IP    EXTERNAL-IP      OS-IMAGE             KERNEL-VERSION       CONTAINER-RUNTIME
lenient-bluebird-3265447277   Ready    <none>   6m53s   v1.29.3   100.97.239.8   138.201.174.85   Ubuntu 22.04.5 LTS   5.15.0-124-generic   containerd://1.7.19
```

Cloudfleet also generates a DNS entry for the service, enabling access via a URL similar to `https://nginx-demo.a8bcca1c-8057-496d-87f8-968d9fa3cc61.northamerica-east-1.cloudfleet.dev:30099/`.
...
```curl https://downloads.cloudfleet.ai/examples/nginx-demo.yaml | kubectl --context a8bcca1c-8057-496d-87f8-968d9fa3cc61/default delete -f -```
...
$  kubectl --context a8bcca1c-8057-496d-87f8-968d9fa3cc61/default get nodes
No resources found
```
