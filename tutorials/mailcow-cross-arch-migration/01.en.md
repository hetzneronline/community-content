...
short_description: "Migrate a production Mailcow installation from an Arm64 (aarch64) server to an x86_64 server with a two-phase rsync, minimal downtime, and no lost mail."
...
* Target server IP: `<target_ip>` (the `x86_64` server you are migrating to)
* Target server user: `holu` (a non-root user with sudo access, as is standard on a fresh server)
* Domain: `<example.com>`
...
Install Docker Engine and the Compose plugin [using the official apt repository](https://docs.docker.com/engine/install/debian/#install-using-the-repository) (recommended for production use, rather than the `get.docker.com` convenience script):
