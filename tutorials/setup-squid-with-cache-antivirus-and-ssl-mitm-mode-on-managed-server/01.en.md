...
date: "2020-07-17"
title: "Setup a Squid proxy with cache, antivirus and SSL-MITM mode on Managed Server"
...
Before we start create a directory for the sources to get a clean home direcory.
```bash
cd
mkdir src
cd src
```
### Step 1.1 - Squid source instalation
...
make -j $(grep "^cpu\ cores" /proc/cpuinfo | uniq | sed s/[^0-9]//g)
make install
cd ..
```
...
### Step 2.5 - Squid configuration cache
...
Then add the following additional configutarion to the end of the file.
...
Now you got a basic introduction how to Install and configure a Squid proxy with a bunch of additional services on a Managed Server. It comes to your attention to secure the proxy, stay up to date and tweak the configuration.
