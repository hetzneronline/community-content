...
```
Now that cURL is installed we can run it. For example, the command to download a file is:
```bash
...
The final missing element is the address of our API itself. It must be at the end of the query. There will be no need to write a flag before it - just the address/link itself. Our command so far:
```bash
...
Next we have the date, the next two lines tell us the content of the request and the last three are server or access information. All this information was provided by using the ``-i`` flag in the cURL command.

The next part of the response is already the actual content from the API. Here - as we can see - we got the data we asked for. We got information about Mr. Joe Doe and his working hours. I think we have the basics of sending requests to the API from the command line covered.
...
### POST, GET, PUT?
Teraz musimy powiedzieć cURLowi jakim typem requesta chcemy wysłać dane. W naszym przypadku będzie to POST. Czyli do kodu dodajemy:
```php
curl_setopt($ch, CURLOPT_POST, true);
```

As if we wanted to execute another type of request like PUT, PATCH, GET, DELETE we need to replace the above code with this. The following code uses a PUT request. Of course, we can change it to the type we currently need.
```php
...
**Array** <br>
The first way is to create a classic array. The data we're going to use presto like this:
```php
$data = array(
	"name" => "John",
	"lastname" => "Doe"
