...
To set up such banner, first head to the `/etc/ssh/sshd_config` file, and uncomment/change the lines to read:  
```
PrintMotd no
PrintLastLog no
```

Now that we eliminated printing this message twice, open the file `/etc/motd`, and paste your preferred legal warning to be shown upon successful connection. The following sample is what _I use_, made up from ASCII art, and a good old CISCO warning:
```
#################################################################
#   ___        _       _    _          _     _                  #
...
## Step 9 - OPTIONAL - Setup SFTP Based Deployment
