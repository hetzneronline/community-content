...
> _**NOTE** If you `ssh` from Windows PowerShell, you might see the contents of motd and last login twice (or even thrice), this is not a problem with `sshd` or `pam` settings, but probably a bug in PowerShell itself. If in doubt, you might want to ensure that things work properly from eg. Git Bash._
