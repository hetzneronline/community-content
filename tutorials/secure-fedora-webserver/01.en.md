...
short_description: "This tutorial will walk you through to create a secure Nginx webserver running on latest Fedora distribution in the Hetzner Cloud"
...
This tutorial will walk through the steps to be taken to secure and publish a newly created Fedora webserver in the [Hetzner Cloud](https://www.hetzner.com/cloud "Open") infrastructure.
...
* Name your server based your workflow's naming convention, eg. `example-host`
* Create the instance
...
_NOTE: Setting up nameservers for registered domains may vary based on your registar, you might have access to a web based console, or might need to ask them to make those changes for you in eg. email._
...
To keep only the latest kernel installed on your system, you can edit the file `/etc/dnf/dnf.conf` modifying the following line to look like this:  
`installonly_limit=2`

### Step 4.1 - OPTIONAL - Install Midnight Commander
...
To have a more friendly theme, one can add an alias to the `~/.bashrc` file:  
...
### Step 5.4 - OPTIONAL - Add a legal banner
...
To set up such banner, first head to the `/etc/ssh/sshd_config` file, and uncomment/change the line to read:  
`PrintMotd no`

Now that we eliminated printing this message twice, open the file `/etc/motd`, and paste your preferred legal warning to be shown upon connection. The following sample is what _I use_, made up from ASCII art, and a good old CISCO warning:
```
#################################################################
#   ___        _       _    _          _     _                  #
...
> **NOTE** I am in no place to give legal advise. If you think your -, or your company's needs reach further, **do** consult with a lawyer (who is well prepared in online statements)! I take no legal responsibility for the above advice, it is merely just a useless friendly reminder.
...
_You can add some temporary content to `/var/www/<your-domain>/html/index.html`_
...
For additional security measurements, consider hardening the server with additional HTTP headers based on your requirements. Some usual ones include:  
```
...
To automatically check if renewal is necessary, and if so, do it we will need a cron job. First run:  
```
...
**NOTE:** If you add any new content, you will also have to notify selinux that it is allowed to serve it to the web, otherwise your requests will fail. You can do this recursively with the previously used command (although it is recommended to be more careful, and provident):  
