...
- On-prem pfSense `<192.168.0.1>` routes internet traffic like normal through its WAN interface while traffic to the VPC is routed via the WireGuard peer `<10.6.210.2>`

#### VPC

- Hetzner's SDN router `<10.0.0.1>` routes VPC and other traffic (`default`) via VPC pfSense `<10.0.128.2>`
- VPC pfSense `<10.0.128.2>` routes internet traffic like normal through its WAN interface while traffic to on-prem is routed via the WireGuard peer `<10.6.210.0>`
...
### In your Hetzner Console session (see above)
