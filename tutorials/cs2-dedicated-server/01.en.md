...
SSH allows you to remotely control your server. Replace `<10.0.0.1>` with your actual server IP from your email.
...
- `YOUR_SERVER_NAME` with your desired server name (e.g., "holu's CS2 Server")
...
```bash
cd ~/cs2-server/game && \
./cs2.sh -dedicated +execconfig server.cfg \
  +sv_setsteamaccount "YOUR_GSLT_TOKEN" \
  +map de_dust2
```
