...
path: "/tutorials/iptables/en"
slug: "iptables"
date: "2020-05-16"
title: "Setting up a packet filter on Linux"
short_description: "This article is about setting up a packet filter on Linux systems."
...
This article is about to understand and use the functions of IPtables.
...
IPtables is a userspace program that is used to configure the netfilter firewall in the Linux kernel.
IPtables is only a tool to configure the firewall integrated in the Linux kernel.

IPtables is divided into two areas, with separate tools for IPv4 and IPv6.
For IPv4 you use `iptables` and for IPv6 `ip6tables` but all following commands can be used with both tools in the same way.

Through the netfilter it is possible:
+ to filter incoming packets before they are routed to the target application.
+ to filter outgoing packets before they leave the computer.
+ to check packets routed through the computer (router).
+ to perform packet manipulation.
...
### Chains
The chains determine when a package is checked, e.g. before it is sent.
A chain can occur in several tables.
...
The most common CLI parameters for creating filter rules are listed here.
...
>Hint:
> The Connection State rules should be high up in the firewall list, otherwise they lose their use. 
...
The configured firewall rules are not automatically persistent. To change this, the following package must be installed `iptables-persistent
...
>
> Note: The blocking is limited to IPv4 addresses.
>
...
In the script above, the variables 'COUNTRIES' can be changed to be shorter according to the countries. When the script is executed it creates two files `input.sh` and `output.sh` which contain the IPtables commands needed to block the IP addresses of the countries.
