...
* `localAPIEndpoint.advertiseAddress: 10.10.0.6` - the IP address of your Control Plane node in the Hetzner Network
...
* `nodeRegistration.kubeletExtraArgs.node-ip: 10.10.0.6` - the IP address of your Control Plane node in the Hetzner Network
* `nodeRegistration.name: k8s-test-master-1` - the name of the Control Plane node
* `apiServer.certSANs: [' k8s-test-master-1.local.example.com ', '10.10.0.6']` - Subject Alternative Names for API service certificate. It can be both, FQDN and local IP address of your Control Pane node
