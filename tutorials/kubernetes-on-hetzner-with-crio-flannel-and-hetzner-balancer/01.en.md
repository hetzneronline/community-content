...
The tutorial contains instructions for installing a Kubernetes cluster version 1.30 using [kubeadm](https://kubernetes.io/docs/setup/production-environment/tools/kubeadm/create-cluster-kubeadm) and [Helm](https://helm.sh). 
By following this tutorial, you will install [CRI-O](https://cri-o.io) as CRI (Container Runtime Interface) and [Flannel](https://github.com/flannel-io/flannel) as CNI (Container Network Interface). 

Cryo was chosen due to the following advantages:
...
As a result, we have a cluster consisting of 1 Control Plane node and 2 Worker nodes. We also have a load balancer operating over HTTPs.

Now you can block all incoming Internet traffic with firewall rules on all cluster nodes and create the remaining resources necessary to run your application on the cluster.
