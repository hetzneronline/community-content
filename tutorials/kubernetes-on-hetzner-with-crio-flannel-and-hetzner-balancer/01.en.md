...
* `controller.hostNetwork: true`- by default, while using `hostNetwork`, name resolution uses the host's DNS. If you wish nginx-controller to keep resolving names inside the Kubernetes network, use `ClusterFirstWithHostNet`
