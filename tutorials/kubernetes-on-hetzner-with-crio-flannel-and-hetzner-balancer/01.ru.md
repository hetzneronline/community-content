...
Теперь мы можем присоединить оставшиеся узлы к кластеру. Запустите команды ниже на всех остальных узлах.
...
* `discovery.bootstrapToken.token: abcdef.0123456789abcdef` - задайте аналогичное значение указанному в [InitConfiguration.yaml файле](#step-4---initializate-cluster)
* `discovery.tlsBootstrapToken: abcdef.0123456789abcdef` - задайте аналогичное значение указанному в [InitConfiguration.yaml файле](#step-4---initializate-cluster)
* `nodeRegistration.criSocket: unix:///var/run/crio/crio.sock` - путь к сокет-файлу CRI-O
...
В этом руководстве в качестве CNI используется [Flannel](https://github.com/flannel-io/flannel). Если вы хотите использовать сетевые политики для организации большего уровня безопасности, вам следует рассмотреть другие плагины.
...
Облачный контроллер Hetzner отвечает за интеграцию кластера Kubernetes с Hetzner API. Установка его на кластер позволяет использовать службы предоставляемые Hetzner. В нашем руководстве мы воспользуемся службой балансировки нагрузки Hetzner.

Создаем Kubernetes секрет содержащий ваш Hetzner Cloud API токен (смотрите пункт "Предварительные требования" в разделе [Вступление](#introduction)) и некий идентификатор сети Hetzner. Идентификатором сети явлются последние цифры указанные в URL указывающим на раздел вашей сети в web-интерфейсе облака Hetzner. Например, для URL https://console.hetzner.cloud/projects/98071/networks/2024666/resources это будет 2024666 
```bash
...
* `controller.hostNetwork: true`- по умолчанию при использовании параметра `hostNetwork` в значении `true`, для разрешения имен используется DNS узла. Если вы хотите, чтобы Ingress Controller продолжал разрешать имена внутри сети Kubernetes используйте `hostNetwor` совместно с `ClusterFirstWithHostNet`
* `controller.kind: DaemonSet` - устанавливает контроллер как DaemonSet
* `controller.service.annotations` - аннотации ниже определют конфигурациюs балансировщика нагрузки Hetzner. Полный список и описание доступных параметров доступен в оффицальном репозитории [облачного контроллера Hetzner](https://github.com/hetznercloud/hcloud-cloud-controller-manager/blob/main/internal/annotation/load_balancer.go)
  * `load-balancer.hetzner.cloud/name: "k8s-test-lb"` - здесь указываем имя балансировщика нагрузки. Имя будет отображаться в соотвествующем разделе web-интерфейса Hetzner облака
  * `load-balancer.hetzner.cloud/location: "fsn1"` - определяет местонахождение дата-центра в котором будет создан балансировщик. Для данного руководства местонахождение должно совпадать с местонахождением виртальных машин вашего кластера Kubernetes
  * `load-balancer.hetzner.cloud/type: "lb11"` - определяет тип блансировщика
  * `load-balancer.hetzner.cloud/ipv6-disabled: "true"` - отключает использование протокола IPv6
  ...
  * `load-balancer.hetzner.cloud/use-private-ip: "true"` - указывает балансировщику использовать локальные IP адреса внутри Hetzner сети для обращения к узлам кластера
  * `load-balancer.hetzner.cloud/protocol: "https"` - определяем протокол по которому будет работать балнсировщик
  * `load-balancer.hetzner.cloud/http-certificates: "certificatename"` - список идентификаторов или названий сертификатов, которые будут использоваться для шифрования трафика между клиентам из Интернета и балансировщиком (смотрите пункт "Предварительные требования" в разделе [Introduction](#introduction))
  * `load-balancer.hetzner.cloud/http-redirect-http: "true"` - включаем перенаправление HTTP на HTTPS
* `controller.service.enableHttp: false` - отключаем HTTP на Kubernete службах контроллера
* `controller.service.targetPorts.https: http` - порт контроллера входящего трафика на который будет перенаправлять запроы балансировщик. Нам нет необходимости использовать шифрование трафика между балансировщиком и кластером Kubernetes, поэтому мы можем указать 80-й (http) порт
