...
- **Hetzner Cloud API token** - Generate one from [Hetzner Cloud Console](https://docs.hetzner.com/cloud/api/getting-started/generating-api-token) with Read & write permissions
...
```bash
export PROJECT_NAME=talos-cluster
...
Create a separate directory and context:
...
hcloud load-balancer create --name $PROJECT_NAME --location hel1 --type lb11

# Add Kubernetes API service
hcloud load-balancer add-service $PROJECT_NAME \
...
hcloud load-balancer attach-to-network $PROJECT_NAME \
  --network $PROJECT_NAME \
  --ip 10.0.0.2

# Target control plane nodes
hcloud load-balancer add-target $PROJECT_NAME \
...
```bash
export LOAD_BALANCER_IP=$(hcloud load-balancer describe $PROJECT_NAME -o format='{{.PublicNet.IPv4.IP}}')
```
...
Install the Hetzner CSI driver to enable dynamic provisioning and management of persistent volumes backed by Hetzner Cloud block storage:
