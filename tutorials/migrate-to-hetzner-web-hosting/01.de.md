...
tags: ["Domain Transfer", "Zone File", "Migration", "Wechsel", "Umstellung", "Auth Code", "Webspace", "FTP", "IMAP", "Sync", "Thunderbird", "FileZilla", "Export", "Import", "CSV", "MySQL", "MariaDB", "PostgreSQL", "phpMyAdmin", "phpPgAdmin", "mysqldump", "pg_dump", "pg_restore", "KonsoleH", "Import"]
...
Ihre Mail-Konten können entweder einzeln angelegt und anschließend mit den Mails aus dem alten Konto befüllt werden, oder Sie nutzen den automatischen Import bzw. senden eine CSV-Datei per Support-Ticket ein, wodurch Ihre Konten angelegt und automatisch importiert werden können (empfohlen bei einer hohen Anzahl Konten).

- [Option 1 - Automatischer Import via KonsoleH](#option-1---automatischer-import-via-konsoleh)
- [Option 2 - Automatischer Import via CSV](#option-2---automatischer-import-via-csv)
- [Option 3 - Einzelner Import](#option-3---einzelner-import)

### Option 1 - Automatischer Import via KonsoleH

Eine ausführliche Anleitung dazu findet sich in den [Hetzner Docs](https://docs.hetzner.com/konsoleh/account-management/email/mailimport).

### Option 2 - Automatischer Import via CSV

Sollte der automatische Import über die Administrationsoberfläche KonsoleH (Option 1) nicht Ihre Anforderungen erfüllen (Sie benötigen bspw. Kopien, Weiterleitungen oder einen Mailbox-Import ohne Mails), können Sie alternativ eine CSV-Datei mit folgendem Aufbau erstellen und dem Hetzner-Support zukommen lassen:
