...
tags: ["Domain Transfer", "Zone File", "Move", "Relocation", "Auth Code", "Webspace", "FTP", "IMAP", "Sync", "Thunderbird", "FileZilla", "Export", "Import", "CSV", "MySQL", "MariaDB", "PostgreSQL", "phpMyAdmin", "phpPgAdmin", "mysqldump", "pg_dump", "pg_restore", "KonsoleH", "Import"]
...
Your mail accounts can either be created individually and then populated with the mails from the old account, or you can use the automatic import via administration interface KonsoleH or CSV file, which allows your accounts to be created and imported automatically (recommended if you have many accounts).

- [Option 1 - Automatic import via KonsoleH](#option-1---automatic-import-via-konsoleh)
- [Option 2 - Automatic import via CSV](#option-2---automatic-import-via-csv)
- [Option 3 - Individual Import](#option-3---individual-import)

### Option 1 - Automatic import via KonsoleH

Detailed instructions for this procedure can be found in the [Hetzner Docs](https://docs.hetzner.com/konsoleh/account-management/email/mailimport).

### Option 2 - Automatic import via CSV

If the automatic import via the administration interface KonsoleH (Option 1) does not meet your requirements (for example, if you need copies, forwarding, or a mailbox import without emails), you can alternatively create a CSV file with the following structure and send it to Hetzner Support:
