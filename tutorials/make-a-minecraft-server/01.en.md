...
Minecraft 1.26.1 requires Java 25 or newer. Ubuntu 24.04 provides OpenJDK 25 LTS:
...
Download the latest server file – either officially from [Mojang](https://www.minecraft.net/en-us/download/server) or [PaperMC](https://papermc.io/downloads) (recommended):
...
Change `eula=false` to `eula=true`. Save with `Ctrl` + `X`, `Y`, Enter.
...
As soon as you see the message, the server has started:
