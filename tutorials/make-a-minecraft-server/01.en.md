...
* **Open the `server.properties` file**<br>
  You can edit the port in the the `server.properties` file. The file is in the same directory as your Minecraft server JAR file.
  
  Edit the `server.properties` file:
  ```bash
  nano server.properties
  ```
  > If you used a different path, change `~/MinecraftServer` respectively.
