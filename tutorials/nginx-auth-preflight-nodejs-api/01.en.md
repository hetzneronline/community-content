...
> _**NOTE:** We run our service in the name of the built-in user `nobody` reserved for vulnerable services, thus assuming we won't have any disc activity - which is true for the current snippet. If you plan to create a more sophisticated version of the service, you might end up needing to create a designated user to run it (eg. to read configurations, or write custom log files), in that case you can look up [`useradd`](https://docs.fedoraproject.org/en-US/fedora/f34/system-administrators-guide/basic-system-configuration/Managing_Users_and_Groups/#s2-users-tools-users-add "Visit") (eg. run `$ useradd -r -N -s /sbin/nologin <your-user>` to create a system user without a group nad home directory that is not allowed to login)._
