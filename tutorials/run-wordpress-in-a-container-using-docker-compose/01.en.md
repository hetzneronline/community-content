...
Skip to [Step 2](#step-2---running-an-existing-wordpress-website-in-docker) if you have existing WordPress website that you want to run in Docker.
...
1. Copy theme directory to the `wp-content/themes`:
...
To make a copy of the WordPress website which can be deployed to a hosting, you need to perform the two steps.
...
> **Note:**
> To run this command, `mariadb` container should be running, i.e. you need to run `docker compose up` first.

* <a name="docker_ps"></a> Replace `wordpress-compose-mariadb-1` with a cotainer name, which can be found by running `docker ps`.
  The `wordpress-compose` prefix corresponds to the name of directory of the `compose.yaml` file, it
  followed by `mariadb` which is the service name.
...
It's now owned by your local user. But container process runs as the `www-data` user.
...
Now, close all the VS Code window and reopen it:
