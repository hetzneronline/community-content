...
MariaDB replication is based on the **binary log (binlog)**. The purpose of binary log is to allow replication, backup and restore databases.
...
To install the latest version of MariaDB, we will need to add their repositories to our servers. If you’re using older version of MariaDB say 5.5, consider upgrading to latest 10.4.5 version using this tutorial.
...
We will now set up in the master server the **exampledb** database
...
Enter the following command to dump **exampledb** database and save them. We will transfer these databases to Slave server later:
