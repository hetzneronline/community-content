...
* Under **Services** select:
  ...
  Select **+ Add certificates**. If `example.com` is configured as DNS zone in Hetzner Console (within the same project or with the option `Share zone with other projects` enabled), you can select "Create certificate". Either `nextcloud.example.com` or a wildcard certificate like `*.example.com` will do.
...
This will only work if `example.com` is configured with the DNS service in Hetzner Console. If not, or you prefer something like caddy or nginx-proxy-manager, you will have to adjust accordingly the way you setup SSL termination.
