...
author_link: "https://paperclip.inc/blog/authors/jannes"
author_img: "https://paperclip.inc/authors/jannes.jpg"
author_description: "Founder and CEO at Paperclip.inc, where the team runs a managed Kubernetes platform on Hetzner and builds the open-source tooling around it."
language: "en"
...
* A running Kubernetes cluster on Hetzner Cloud (control plane reachable, a CNI installed). Tutorials such as the ones for `kubeadm` or kube-hetzner get you here.
...
* A Hetzner Cloud [API token](https://docs.hetzner.com/cloud/api/getting-started/generating-api-token) with read and write access, created in the [Cloud Console](https://console.hetzner.cloud/).
...
Within a couple of minutes a new Hetzner server is created, joins the cluster, and the `inflate` pods schedule onto it. Confirm the server in the [Cloud Console](https://console.hetzner.cloud/) or with `hcloud server list`.
...
Hetzner's Ampere Arm (CAX) servers are often the lowest-cost option. Add a second `NodePool` that only offers `arm64`, and let your Arm-capable workloads land there:
