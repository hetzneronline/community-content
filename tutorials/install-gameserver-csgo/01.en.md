...
path: "/tutorials/install-gameserver-csgo/en"
slug: "install-gameserver-csgo"
date: "2019-03-28"
title: "Installing a CSGO Gameserver"
short_description: "Describes the installation of a CSGO server on Linux."
...
This article is about installing and configuring a CSGO Gameserver on Linux systems.

**Requirements**

+ An x86 / x64 compatible system
+ Ubuntu 16.04 LTS
+ Installed [`steamcmd`](../install-steamcmd/en)

## Step 1 - Preparations

Creating the CSGO server folder, here are the server files later.
```bash
mkdir /opt/csgo/
...
function update_server {
    stop_server
    su --shell /bin/bash $USER -c "steamcmd +login anonymous +force_install_dir $DIR +app_update 740 +quit"
    start_server
}
...
```bash
0 4 * * * systemctl stop csgo && /opt/steamcmd/csgo.sh update && systemctl start csgo >/dev/null 2>&1
```
This entry would be executed every day at 4:00 AM.

## Troubleshooting

Should there be any problems connecting to the `screen`.
```bash
Cannot open your terminal '/dev/pts/0' - please check.
```
Execute the following command in the current session.
```bash
script /dev/null
```
...
Installing a CSGO server and setting up management tools.
