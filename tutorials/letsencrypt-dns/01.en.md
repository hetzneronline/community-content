...
Note that the [Hetzner firewall for dedicated servers](https://docs.hetzner.com/robot/dedicated-server/firewall) is stateless. As a result,  packets from Let's Encrypt will be blocked unless corresponding rules are configured. If you have the firewall, make sure the ingoing ephemeral port range is allowed.
...
If your domain, from the viewpoint of Hetzner DNS panel / Hetzner API, includes a second-level designation, like `example.co.uk`, update the value of `domain_name` at the beginning of both files from `1,2` to `1,2,3`:
