...
`enp7s0` is the interface name of our private Network.
...
In this example, we are using Hetzner DNS servers, but you can also use other DNS servers. The cloud-config will change the `/etc/systemd/resolved.conf` file to this:
...
> Disable the `hc-utils` package. Otherwise there's a risk, that two DHCP clients will compete over the configuration, which will cause side effects or outages. (See [Hetzner docs](https://docs.hetzner.com/networking/networks/server-configuration#manual-configuration-via-dhcp))
