...
And you also need DNS records pointing to IP address of your server.
...
Here find `[[outputs.influxdb]]` section, uncomment `username` and `password`, and also specify the password you have set on [Step 1 - Install InfluxDB](#step-1---install-influxdb).
...
Create file `/etc/telegraf/telegraf.d/netquery.conf`, and paste the following content:
...
If you only use Grafana/InfluxDB on your local machine and not expose it to the Internet, you would not need to secure your connection.
...
Click a big blue "Add data source" button to add a new data source, and then select `InfluxDB` in a `Time series databases` section.
...
Enter dashboard ID `14419` and click `Load` button.
...
Now you can see HC Monitor dashboard in action.
...
Here we replace the `http://127.0.0.1:8086` with `https://influx.example.com:8086`.
