...
apt install sshfs
```
> __Note 1:__ [bash-completion](https://github.com/scop/bash-completion) works well with all the commands and manipulations that you need in this tutorial. If you enter `systemctl <TAB><TAB>` you should see all commands available. Completion works also for filenames involved in _systemd_ later configuration, so think `<TAB><TAB>` to avoid headache.

> __Note 2:__ in this tutorial commands runnable as unprivileged user start with the `$` prompt. Commands that need to run as root user start with the `#` prompt.
...
> Note that `noauto` is useless now that _systemd_ manages the mount ([reference](https://www.suse.com/support/kb/doc/?id=000020402)). But you can keep it in case you decide to go back for a manual mount.
...
```
> Note: you can use shell completion `systemctl status mnt<TAB><TAB>`
...
> Note the `--type=automount` argument used to filter the units type. If you remove it you will dive into the heart of your old loved SysV system...
...
> Note that this unit file exists really and it's path is `/run/systemd/generator/mnt-mymountpoint.automount` (as the `cat` command said in comment). Do not modify it, it's Automatically generated by systemd-fstab-generator (as the `cat` command said in comment ;) But you could copy it to `/etc/systemd/system/` and later tweak it...
...
Note the beauty of the result: now all is yellow to show that the mount is active and currently mounted. The command returns also a log of the operations on the mount, with the command that triggered the automount. Here the `ls` command (with PID number 2523562)

> Note: the trigger log is shown only to the root user. If you launch the `status` command as a normal user, you will just see the unit status, but not the events log.
