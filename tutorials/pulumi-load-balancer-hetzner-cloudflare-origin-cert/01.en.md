...
Cloudflare shows the certificate and private key only once. Copy them into two local files:

```bash
# create cert.pem — paste the certificate content
nano cert.pem

# create key.pem — paste the private key content
nano key.pem
```

> Keep `key.pem` safe. Cloudflare does not store the private key and cannot show it again.
...
DOMAIN=<domain>
TLS_CERT=<paste origin certificate content here>
TLS_PRIVATE_KEY=<paste private key content here>
...
> `HCLOUD_TOKEN` and `CLOUDFLARE_API_TOKEN` are picked up automatically by the Hetzner and Cloudflare providers from the environment — no `pulumi config set` needed for them. `DOMAIN`, `CLOUDFLARE_ZONE_ID`, `TLS_CERT`, and `TLS_PRIVATE_KEY` are read directly by the Pulumi program from the environment. The certificate and private key are wrapped in `pulumi.secret()` in the code, so they are encrypted in the state file.

> If you already have an SSH key in your Hetzner project, use `pulumi config set sshKeyName <key-name>` instead of `sshPublicKeyPath`.
...
const tlsCertificate = config.requireSecret("tlsCertificate");
const tlsPrivateKey = config.requireSecret("tlsPrivateKey");
...
const hcloudProvider = new hcloud.Provider("hcloud", { token: hcloudToken });
